...
fprintf(stderr, "-------->> 6.To Send CSAPP_ABIHM_BICH_BM_HELLO \n");
fprintf(stderr, "-------->> 7.To Send CSAPP_ABIHM_BICH_BIC_BSC_ALARM_IND \n");
fprintf(stderr, "-------->> 8.To Send CSAPP_ABIHM_BICH_FAILURE_EVENT_REPORT \n");
fprintf(stderr, "<<-------- 9. To_Receive data \n");
...
switch(choice)
{
	 ...
	 case 6 :
		fprintf(stderr, "-------->>Enter TEI, for which  BM_HELLO_MSG is  - ");
                ...
                break;
	 case 7 :
		fprintf(stderr, "-------->>Enter TEI, for which  BIC_BSC_ALARM_IND_MSG is  - ");
                scanf("%d",&tei);
                bufferPtr = Build_CsappAbihmBichBicBscAlarmMsg(tei, &size);
                ch = 1 ;
                break;
	 case 8 :
		fprintf(stderr, "-------->>Enter TEI, for which  FAILURE_EVENT_REPORT is  - ");
                scanf("%d",&tei);
                bufferPtr = Build_CsappAbihmBichFailureEventReport(tei, &size);
                ch = 1 ;
                break;
	 case 9 :
		ReceiveData() ;
