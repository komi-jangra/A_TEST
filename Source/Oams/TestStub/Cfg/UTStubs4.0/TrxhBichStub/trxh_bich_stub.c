...
void bichReqHandler()
{
   ...
   while(1)
   {
	 ...
	 case OAMS_CFG_BICH_LOCK_REQ:
	    printf("Received Msg : OAMS_CFG_BICH_LOCK_REQ");
	    lockBicReqHandler();
	    break;
	 case OAMS_CFG_BICH_UNLOCK_REQ:
	    printf("Received Msg : OAMS_CFG_BICH_UNLOCK_REQ");
	    unlockBicReqHandler();
	    break;
    case OAMS_CFG_BICH_BTS_RESET:
       printf("Received Msg : OAMS_CFG_BICH_BTS_RESET");
	    resetBicReqHandler();
	    break;
      }
      DalocMsgBuf(cfgMsgBuf);
...
#endif
#if 1
void lockBicReqHandler()	/* For Successfull Result */
{
   I_U8 ch;
   OamsCfgBichLockReq *oamsCfgBichLockReqPtr = NULL;
   OamsBichCfgLockResp oamsBichCfgLockResp;
   oamsCfgBichLockReqPtr = (OamsCfgBichAddBicReq *) cfgMsgBuf;
   printf("BIC Id (from Cfg):: %d \n" , oamsCfgBichLockReqPtr->bicId);
   oamsBichCfgLockResp.bicId = oamsCfgBichLockReqPtr->bicId;
   printf("Enter the Result ");
   scanf("%d",&ch);
   if(ch == 1)
   {
     oamsBichCfgLockResp.result = OAMS_RSLT_SUCCESS;
   }
   else
   {
     oamsBichCfgLockResp.result = OAMS_RSLT_FAILURE;
     oamsBichCfgLockResp.cause = 1;
   } 
   cfgBichStubSendMessage(&oamsBichCfgLockResp, sizeof(OamsBichCfgLockResp), ENT_OAMS_BICH, ENT_OAMS_CFG, OAMS_BICH_CFG_LOCK_RESP,0);
}
#endif
#if 1
void unlockBicReqHandler()	/* For Successfull Result */
{
   I_U8 ch;
   OamsCfgBichUnlockReq *oamsCfgBichUnlockReqPtr = NULL;
   OamsBichCfgUnlockResp oamsBichCfgUnlockResp;
   oamsCfgBichUnlockReqPtr = (OamsCfgBichUnlockReq *) cfgMsgBuf;
   printf("BIC Id (from Cfg):: %d \n" , oamsCfgBichUnlockReqPtr->bicId);
   oamsBichCfgUnlockResp.bicId = oamsCfgBichUnlockReqPtr->bicId;
   printf("Enter the Result ");
   scanf("%d",&ch);
   if(ch == 1)
   {
     oamsBichCfgUnlockResp.result = OAMS_RSLT_SUCCESS;
   }
   else
   {
     oamsBichCfgUnlockResp.result = OAMS_RSLT_FAILURE;
     oamsBichCfgUnlockResp.cause = 1;
   } 

   cfgBichStubSendMessage(&oamsBichCfgUnlockResp, sizeof(OamsBichCfgUnlockResp), ENT_OAMS_BICH, ENT_OAMS_CFG, OAMS_BICH_CFG_UNLOCK_RESP,0);
}
#endif
#if 1
void resetBicReqHandler()	/* For Successfull Result */
{
   OamsCfgBichBtsReset *oamsCfgBichBtsResetPtr = NULL;
   oamsCfgBichBtsResetPtr = (OamsCfgBichBtsReset *) cfgMsgBuf;
   printf("BIC Id (from Cfg):: %d \n" , oamsCfgBichBtsResetPtr->bicId);
}
#endif
