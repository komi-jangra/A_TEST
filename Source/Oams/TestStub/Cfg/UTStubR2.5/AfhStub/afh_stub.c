...
main()
{
   ...
   while (1) {
      ...
      {  
         ...
         printf("\nAFH_STUB :  Msg. size.(%d)",msgSize);
         printf("\nAFH_STUB :  Msg. Rcvd.(%d)",msgType);
         printf("\nAFH_STUB :  src (%d)", ((SysHdr *)(rcvPtr))->src.entity);
         printf("\nAFH_STUB :  src (%d)", ((SysHdr *)(rcvPtr))->dest.entity);

         switch(msgType)
         {
  	    case OAMS_CFG_AFH_ALARM_IND:
            {
               LOG_PRINT(LOG_INFO,"\nRecieved OAMS_CFG_AFH_ALARM_IND from Cfg..\n");
               OamsCfgAfhSysAlarm *sysAlarmPtr = (OamsCfgAfhSysAlarm *)rcvPtr;
               LOG_PRINT(LOG_INFO,"\nAlarm Id: %d ",sysAlarmPtr->sysAlarm.sysalarmId);
               LOG_PRINT(LOG_INFO,"\nInfo3 : %d ",sysAlarmPtr->sysAlarm.info3);
               LOG_PRINT(LOG_INFO,"\nKey1 : %d ",sysAlarmPtr->sysAlarm.key1);
               LOG_PRINT(LOG_INFO,"\nAlarmLevel : %d ",sysAlarmPtr->sysAlarm.alarmLevel);
               LOG_PRINT(LOG_INFO,"\nInfoStrLen : %d ",sysAlarmPtr->sysAlarm.infoStrLen);
               LOG_PRINT(LOG_INFO,"\nInfoString : %s ",sysAlarmPtr->sysAlarm.infoString);
               break;
            }

            default:
               {
                  LOG_PRINT(LOG_INFO," \nNo Message handling for the message Type : %d \n",(((SysHdr*) rcvPtr)->msgType));
                  break;
               }
         }//switch
         DalocMsgBuf(rcvPtr);
