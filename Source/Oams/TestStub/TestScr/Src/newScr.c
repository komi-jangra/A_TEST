...
result = dbConnect();
  ...
  I_S32         dbSts=DBLIB_SUCCESS,dbStsPs=DBLIB_SUCCESS,i;
  I_Void        *outData,*tmpData1,*tmpData2,*tmpData3,*tmpData4,*tmpData5,*tmpData6,*tmpData7,*tmpData8,*tmpData9,*tmpData10,*tmpData11;
  BicTableApi *tmpData;
  I_CHAR        tmpStr[20];
  ...
  BicStatusTableIndices bicStatus;
	...
	PsCellStatusTableIndices tmpPsCellStsTableKey;
	 	time_t now;
 	 	time(&now);
tmpCnt1=0;
 fprintf(stderr,"-----------------------------------------------------------------------------------------------------------------------------------------\n");
...
*/
        fprintf(stderr,"%19s#%9s#%6s#%6s#%16s#%7s#%s#%7s#%8s#%7s#%7s#%8s#%s#%8s#%s#\n","BtsName","BtsId","AbisS","BtsS","IpAddr","CellId","(Band)","AntId","BArfcn","CellS","PsCellS","BTrxS",":PId","NBTrxS",":Pid");
 fprintf(stderr,"-----------------------------------------------------------------------------------------------------------------------------------------\n");
          ...
          if(dbSts == DBLIB_SUCCESS && outRows2 > 0)
          {
            ...
            tmpCellTable = (CellPerTableApi *)((I_U8*)tmpData2 + (tmpCnt2 *outSize2));
              if(tmpAntId == tmpCellTable->antennaId)
              {
                ...
                tmpPsCellStsTableKey.lac    =tmpCellTable->lac;
                tmpPsCellStsTableKey.cellId =tmpCellTable->cellId;
								dbStsPs = getPsCellStatusTable(&tmpPsCellStsTableKey,(PsCellStatusTableApi **)&tmpData11);
                ...
                if(dbSts == DBLIB_SUCCESS) 
                {
                  if(((CellNonPerTableApi *) tmpData3)[0].oprState == 1)  /*1=ENABLED 0=DISABLED*/
                  {
                    ...
                    fprintf(stderr,"%7s#","LENB"); 
										...
										if(dbStsPs == DBLIB_SUCCESS)
										{
														if(((PsCellStatusTableApi *) tmpData11)[0].oprState == 1)
														{
																		if(tmpCellTable->adminState == 1)
																						fprintf(stderr,"%7s#","UENB");
																		else
														...
														{
																		if(tmpCellTable->adminState == 1)
																						fprintf(stderr,"%7s#","UDIS");
																		else
																						fprintf(stderr,"%7s#","LDIS");
