...
/* Add for PCU Rel 2.5  */
I_PVoid Build_OamsCfgCellhCsNeighCellsChange(I_U32, I_U32, I_S32 *);
I_PVoid Build_OamsCfgGrhmStartChanAlloc(I_S32 *);
I_PVoid Build_OamsCfgGrhmStopChanAlloc(I_S32 *);
I_PVoid Build_OamsCfgGrhmStartStaticSwitching(I_S32 *);
/* Add for PCU Rel 2.5  */
...
fprintf(stderr, "-------->> 3.To Send OAMS_CFG_CELLH_BLOCK_IND \n");
fprintf(stderr, "-------->> 4.To Send OAMS_CFG_CELLH_CELL_BAR_ACCESS \n");

fprintf(stderr, "-------->> 5.To Send OAMS_CFG_CELLH_CSNEIGHCELLS_CHANGE \n");

fprintf(stderr, "-------->> 6.To Send OAMS_CFG_TRXH_UNLOCK_REQ \n");
fprintf(stderr, "-------->> 7.To Send OAMS_CFG_TRXH_LOCK_REQ \n");


fprintf(stderr, "-------->> 8.To Send OAMS_CFG_TRXH_ADD_TRX_REQ \n");
fprintf(stderr, "-------->> 9.To Send OAMS_CFG_TRXH_DEL_TRX_REQ \n");

fprintf(stderr, "-------->> 10.To Send OAMS_CFG_BICH_ADD_BIC_REQ \n");
fprintf(stderr, "-------->>11.To Send OAMS_CFG_BICH_DEL_BIC_REQ \n");
fprintf(stderr, "-------->>12.To Send OAMS_CFG_BICH_INIT_ABIS_E1 \n");
fprintf(stderr, "-------->>13.To Send OAMS_CFG_BICH_UPDATE_ABIS_E1 \n");

fprintf(stderr, "-------->>14.To Send OAMS_CFG_BICH_LOCK_REQ \n");
fprintf(stderr, "-------->>15.To Send OAMS_CFG_BICH_UNLOCK_REQ \n");
fprintf(stderr, "-------->>16.To Send OAMS_CFG_BICH_ADD_TRX_INDICATION \n");
fprintf(stderr, "-------->>17.To Send OAMS_CFG_BICH_DEL_TRX_INDICATION \n");
fprintf(stderr, "-------->>18 To Send OAMS_CFG_BICH_BTS_RESET \n");
fprintf(stderr, "-------->>19 To Send OAMS_CFG_GRHM_START_CHAN_ALLOC \n");
fprintf(stderr, "-------->>20 To Send OAMS_CFG_GRHM_STOP_CHAN_ALLOC \n");
fprintf(stderr, "-------->>21 To Send OAMS_CFG_GRHM_START_STATIC_SWITCHING \n");
fprintf(stderr, "<<--------22. To_Receive data \n");
...
//		printf("\n----Value of CellId and Lac is - %d %d\n", cellId,lac); 
	...
	case 5 :
		...
		fprintf(stderr,"--------UNLOCK Req for Lac - \n");
                scanf("%d",&lac);
		bufferPtr = Build_OamsCfgCellhCsNeighCellsChange(cellId, lac, &size);
		break;
		...
		scanf("%d",&e1Id);
                ...
                break;
        case 14 :
		fprintf(stderr, "\n--------Enter bicId, for which this Lock BIC Req is  - ");
		scanf("%d",&bicId);
                //bicId=0xffffffff;
                bufferPtr = Build_OamsCfgBichLockBicReqMsg(bicId, &size);
                break;
        case 15 :
		fprintf(stderr, "\n--------Enter bicId, for which this UnLock BIC Req is  - ");
		scanf("%d",&bicId);
                //bicId=0xffffffff;
                bufferPtr = Build_OamsCfgBichUnlockBicReqMsg(bicId, &size);
                break;
        case 16 :
		fprintf(stderr, "\n--------Enter bicId, for which Add TRX Indication is  - ");
		scanf("%d",&bicId);
		fprintf(stderr, "\n--------Enter trxId, for which Add TRX Indication is  - ");
		scanf("%d",&trxId);
                //bicId=0xffffffff;
                bufferPtr = Build_OamsCfgBichAddTrxIndMsg(bicId, trxId, &size);
                break;
        case 17 :
		fprintf(stderr, "\n--------Enter bicId, for which DEL TRX Indication is  - ");
		scanf("%d",&bicId);
		fprintf(stderr, "\n--------Enter trxId, for which DEL TRX Indication is  - ");
		scanf("%d",&trxId);
                //bicId=0xffffffff;
                bufferPtr = Build_OamsCfgBichDelTrxIndMsg(bicId, trxId, &size);
                break;
        case 18 :
		fprintf(stderr, "\n--------Enter bicId, for which BM RESET Req is  - ");
		scanf("%d",&bicId);
                //bicId=0xffffffff;
                bufferPtr = Build_OamsCfgBichBmResetMsg(bicId, &size);
                break;
	case 19 :
		bufferPtr = Build_OamsCfgGrhmStartChanAlloc(&size);
		break;
	case 20 :
		bufferPtr = Build_OamsCfgGrhmStopChanAlloc(&size);
		break;
	case 21 :
		bufferPtr = Build_OamsCfgGrhmStartStaticSwitching(&size);
		break;
	case 22 :
		ReceiveData() ;
	        break ;		
	...
	if (choice ==1  || choice == 2 ||  choice == 3 || choice == 4 || choice ==5 \
            || choice == 6 || choice == 7 || choice == 8 || choice == 9 || choice == 10 || choice == 11 || choice == 12 || choice == 13 || choice == 14 || choice == 15 || choice == 16 || choice == 17 || choice ==18 || choice==19 || choice==20 || choice==21)
	{
...
I_PVoid Build_OamsCfgCellhCsNeighCellsChange(I_U32 cellId, I_U32 lac, I_S32 *size)
{
	OamsCfgCellhCsNeighCellsChange  *unlockReqMsg ;
	I_U16 len=0;

	len = sizeof(OamsCfgCellhCsNeighCellsChange);
	if((unlockReqMsg = (OamsCfgCellhCsNeighCellsChange *)AlocMsgBuf(len) ) == (I_Void *)NULL)
        {
           fprintf(stderr,"Message Buffer Aloc. Failure for  App Begin\n");
           exit(0);
        }
		 
	unlockReqMsg->cellId = cellId ;
	unlockReqMsg->lac = lac;
	*size=len;
	   
	FILL_SYS_HDR(&(unlockReqMsg->sysHdr),OAMS_CFG_CELLH_CSNEIGHCELLS_CHANGE, 1,PRCR_CP, ENT_OAMS_CFG ,0,PRCR_CP, ENT_OAMS_CELLH,0);
...
{
        OamsCfgCellhBlockInd    *shutDwnReqMsg ;

        //I_U32 dinstid=0;
        I_U16 len=0;
       // ObjectId cellId=3500;
	//Lac   lac=3000;
	//printf(" Entered Inst Id = %d \n", dinstid);

        len = sizeof(OamsCfgCellhBlockInd);
        if((shutDwnReqMsg = (OamsCfgCellhBlockInd *)AlocMsgBuf(len) ) == (I_Void *)NULL)
        {
           fprintf(stderr,"Message Buffer Aloc. Failure for  App Begin\n");
           exit(0);
        }

       // fprintf(stderr,"--------SHUTDOWN Req for CellId - \n");
       // scanf("%d",&cellId);
       // fprintf(stderr,"--------SHUTDOWN Req for Lac - \n");
       // scanf("%d",&lac);

        //Filling OAMS_CFG_TRXH_LOCK_REQ Messages
        shutDwnReqMsg->objId = 3500 ;
        shutDwnReqMsg->lac = 3000 ;
	*size=len;

        FILL_SYS_HDR(&(shutDwnReqMsg->sysHdr),OAMS_CFG_CELLH_BLOCK_IND, 1,PRCR_CP, ENT_OAMS_CFG ,0,PRCR_CP, ENT_OAMS_CELLH,0);
...
}// Build_OamsCfgCellhBlockInd Complete
...
//        fprintf(stderr,"\n--------Enter CellBar status -");
//        scanf("%u",&cellBar);

        //Filling OAMS_CFG_TRXH_LOCK_REQ Messages
        cellBarMsg->cellId = 3500 ;
        cellBarMsg->lac = 3000 ;
...
I_PVoid Build_OamsCfgTrxhAddTrxReqMsg(I_U32 ptrxId, I_S32 *size)
{
	...
	inData.trxTei=ptrxId-5;
	inData.e1Id=1;
	inData.firstTsTraffic =ptrxId-5;
	inData.secondTsTraffic=inData.firstTsTraffic +1;
	inData.lapdTs=inData.firstTsTraffic -1 ;
	inData.lapdLinkStatus=0;
	
	errCode=insertTrxTsTeiTable(&inData);
        if(errCode !=ZERO)
        {
	   fprintf(stderr, "\n--------Error in insertinsertTrxTsTeiTable ");
	}
	fprintf(stderr, "TrxTsTeiTableApi(e1Id %u ptrxId %d trxTei %d )",inData.e1Id,inData.ptrxId,inData.trxTei);
...
I_PVoid Build_OamsCfgBichAddBicReqMsg(I_U32 bicId, I_S32 *size)
{
	...
	scanf("%d",&tsVal);
        ...
        fprintf(stderr, "\n--------Enter BtsType (BIC(1)/BTS(2)), for which this ADD Req is  - ");
	scanf("%d",&btsType);
        addBicReqMsg->btsType =btsType;
        fprintf(stderr, "\n--------Enter AdminState (Locked(0)/Unlocked(1)), for which this ADD Req is  - ");
	scanf("%d",&(addBicReqMsg->adminState));

        fprintf(stderr, "\n--------Enter ApplTei, for which this ADD Req is  - ");
	scanf("%d",&bicApplTei);
        addBicReqMsg->applTei =bicApplTei;
	...
	*size=len;
	...
	bicTrxTimeSlotUsageTableApi.lapdTs=(I_U8)tsVal;
	...
	teiMapData.btsId=bicId;
	teiMapData.bicTei=e1Id+tsVal-1;
	errCode=insertBicE1TeiMapTable(&teiMapData);
        if(errCode !=ZERO)
        {
	   fprintf(stderr, "\n--------Error in insertBicE1TeiMapTable ");
	}
	fprintf(stderr, "insertBicE1TeiMapTable(e1Id %d tsVal %d btsId %d bicTei %u)",e1Id,tsVal,bicId,teiMapData.bicTei);
...
I_PVoid Build_OamsCfgGrhmStartChanAlloc(I_S32 *size)
{
	OamsCfgGrhmStartChanAlloc    *startChanAllocMsg ;
	I_U16 len=0;

	len = sizeof(OamsCfgGrhmStartChanAlloc);
	if((startChanAllocMsg = (OamsCfgGrhmStartChanAlloc *)AlocMsgBuf(len) ) == (I_Void *)NULL)
        {
           fprintf(stderr,"Message Buffer Aloc. Failure for  App Begin\n");
           exit(0);
        }
	...
	*size=len;
	...
	FILL_SYS_HDR(&(startChanAllocMsg->sysHdr),OAMS_CFG_GRHM_START_CHAN_ALLOC, 1,PRCR_CP, ENT_OAMS_CFG ,0,PRCR_CP, ENT_CSAPP_GRHM,0);
        startChanAllocMsg->reason=OAMS_RSN_TRAUUP;
	fprintf(stderr,"App Begin (len = %d : 0x%x) \n", len, len);
	fprintf(stderr,"App Begin (*size = %d : 0x%x) \n", *size, *size);

	return  startChanAllocMsg;
			
}// Build_OamsCfgGrhmStartChanAlloc Complete


I_PVoid Build_OamsCfgGrhmStopChanAlloc(I_S32 *size)
{
	OamsCfgGrhmStopChanAlloc    *stopChanAllocMsg ;
	I_U16 len=0;

	len = sizeof(OamsCfgGrhmStopChanAlloc);
	if((stopChanAllocMsg = (OamsCfgGrhmStopChanAlloc *)AlocMsgBuf(len) ) == (I_Void *)NULL)
        {
           fprintf(stderr,"Message Buffer Aloc. Failure for  App Begin\n");
           exit(0);
        }
	...
	*size=len;
	...
	FILL_SYS_HDR(&(stopChanAllocMsg->sysHdr),OAMS_CFG_GRHM_STOP_CHAN_ALLOC, 1,PRCR_CP, ENT_OAMS_CFG ,0,PRCR_CP, ENT_CSAPP_GRHM,0);
        stopChanAllocMsg->reason=OAMS_RSN_TRAUDOWN;

	fprintf(stderr,"App Begin (len = %d : 0x%x) \n", len, len);
	fprintf(stderr,"App Begin (*size = %d : 0x%x) \n", *size, *size);

	return  stopChanAllocMsg;
}
I_PVoid Build_OamsCfgGrhmStartStaticSwitching(I_S32 *size)
{
        SysHdr    *sysHdr;
	I_U16 len=0;

	len = sizeof(SysHdr);
	if((sysHdr = (SysHdr *)AlocMsgBuf(len) ) == (I_Void *)NULL)
        {
           fprintf(stderr,"Message Buffer Aloc. Failure for  App Begin\n");
           exit(0);
        }
	...
	*size=len;
	...
	FILL_SYS_HDR(sysHdr,OAMS_CFG_GRHM_START_STATIC_SWITCHING, 1,PRCR_CP, ENT_OAMS_CFG ,0,PRCR_CP, ENT_CSAPP_GRHM,0);
