...
I_PVoid Build_OamsBichTrxhIpLinkDown(I_S32 *,I_U32 ptrxId);
I_PVoid Build_OamsBichTrxhIpLinkUp(I_S32 *,I_U32 ptrxId);
...
fprintf(stderr, "-------->> 4.To Send OAMS_BICH_TRXH_IP_LINK_DOWN \n");
fprintf(stderr, "-------->> 5.To Send OAMS_BICH_TRXH_IP_LINK_UP \n");
fprintf(stderr, "<<-------- 6. To_Receive data \n");
scanf("%d", &choice);
...
switch(choice)
{
	...
	case 4 :
   		fprintf(stderr,"--------IP_LINK_DOWN for PtrxId - \n");
		scanf("%d",&ptrxId);
		printf("\n----Value of PtrxId is - %d \n", ptrxId); 
		bufferPtr = Build_OamsBichTrxhIpLinkDown(&size, ptrxId);
		break;

	case 5 :
   		fprintf(stderr,"--------IP_LINK_UP for PtrxId - \n");
		scanf("%d",&ptrxId);
		printf("\n----Value of PtrxId is - %d \n", ptrxId); 
		bufferPtr = Build_OamsBichTrxhIpLinkUp(&size, ptrxId);
		break;
...
I_PVoid Build_OamsBichTrxhIpLinkDown(I_S32 *size, I_U32 ptrxId)
{
        OamsBichTrxhIpLinkDown    *ptr ;

        I_U16 len=0;
        len = sizeof(OamsBichTrxhIpLinkDown);
        if((ptr = (OamsBichTrxhIpLinkDown *)AlocMsgBuf(len) ) == (I_Void *)NULL)
        {
           fprintf(stderr,"Message Buffer Aloc. Failure for  App Begin\n");
           exit(0);
        }
        ...
        fprintf(stderr, " Given PtrxId =%d " ,ptrxId);
        *size=len;

        FILL_SYS_HDR(&(ptr->sysHdr),OAMS_BICH_TRXH_IP_LINK_DOWN, 1, PRCR_CP, ENT_OAMS_BICH,0,PRCR_CP, ENT_OAMS_TRXH,0);
...
}// Build_OamsBichTrxhIpLinkDown Complete

I_PVoid Build_OamsBichTrxhIpLinkUp(I_S32 *size, I_U32 ptrxId)
{
        OamsBichTrxhIpLinkUp    *ptr ;

        I_U16 len=0;
        len = sizeof(OamsBichTrxhIpLinkUp);
        if((ptr = (OamsBichTrxhIpLinkUp *)AlocMsgBuf(len) ) == (I_Void *)NULL)
        {
           fprintf(stderr,"Message Buffer Aloc. Failure for  App Begin\n");
           exit(0);
        }
        ...
        *size=len;
        ...
        FILL_SYS_HDR(&(ptr->sysHdr),OAMS_BICH_TRXH_IP_LINK_UP, 1, PRCR_CP, ENT_OAMS_BICH,0,PRCR_CP, ENT_OAMS_TRXH,0);
...
}// Build_OamsBichTrxhIpLinkUp Complete
