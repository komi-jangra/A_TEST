...
srcId = (int)ENT_PSAPP_GRRM ;

if(RegisterWithIpcFrmwrk(srcId, (I_S8 *)"ENT_PSAPP_GRRM")!=0)
{
...
void ReceiveData()
{
	...
	while(1)
	{
		rcvPtr = (I_PVoid)RecvMsg((int)(ENT_PSAPP_GRRM), &msgSize);
	...
	switch (msgType)
	{
		case OAMS_TRXH_PCU_PDCH_AVAIL:
			fprintf(stderr," PDCH AVAIL received from TRXH\n") ;
			break ;
