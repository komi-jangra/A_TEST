 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-mtp3.c,v 1.1.1.1 2007-10-08 11:12:15 bsccs2 Exp $
 *
...
#include <sched.h>
#ident "$Id: itu-mtp3.c,v 1.1.1.1 2007-10-08 11:12:15 bsccs2 Exp $"
...
static int MarkAllRoutesUnAvailable(ITS_POINTER entry, void *in, void *out);
/*
 * local data
 */
...
int  __ccitt_leaveTmr = 2000;
/*
 * behavior control
 */
ITS_BOOLEAN MTP3_CCITT_HandleSpareNIC = ITS_TRUE;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
MTP3_EnableUserPart(ITS_OCTET up, ITS_USHORT id)
{
...
//    MTP3_DEBUG(("ROUTE_LockForWrite:File %s Line %d",__FILE__,__LINE__));
            ...
            if (wasUp && !isUp)
            {
                     ...
                     count++, tmp = DSMQUEUE_NEXT(link->linkSet->routes, tmp) )
                {
                    ...
                    if (allDown)
                    {
                        ...
                        MTP3_Alarm_CCITT(2401, __FILE__, __LINE__, "affected point code %x", rinfo->basic.dpc);
                     ...
                     count++, tmp = DSMQUEUE_NEXT(link->linkSet->routes, tmp) )
                {
                    ...
                    if (firstUp)
                    {
                        if (rinfo->dest)
                        {
                            ...
                            {
                                if (rinfo->dest->entries[i]->linkSet->linkSet != link->linkSet->linkSet &&
...
IMTP3_Initialize_CCITT(ITS_SS7_HANDLE handle)
{
    ...
    if (ITS_RedundancyClass != NULL)
    {
        ...
        if (REDUNDANCY_CONFIGURED_TYPE(ITS_RedundancyClass) == ITS_ACTIVE_ACTIVE)
        {            
    ...
    if ( (routeRWLock = ROUTE_RWLockCreate() ) == NULL)
    {
        ...
        MTP3_CRITICAL(("MTP3_Initialize: ROUTE_RWLockCreate SUCCESS\n"));
...
HandleTimerEvent(ITS_TimerData *td)
{
    ...
    ret = ITS_SUCCESS;
        MTP3_DEBUG(("MTP3_Main:EXPIRE handle MTP3 TIMER msg.: SERIAL [%d]\n",td->serial));
    ...
    case MTP3_T1:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T1  TIMER msg.\n"));
    ...
    case MTP3_T2:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T2  TIMER msg.\n"));
    ...
    case MTP3_T3:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T3  TIMER msg.\n"));
    ...
    case MTP3_T4:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T4  TIMER msg.\n"));
    ...
    case MTP3_T5:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T5  TIMER msg.\n"));
    ...
    case MTP3_T6:
        MTP3_DEBUG(("MTP3_Main: handle T6  TIMER msg.\n"));
    ...
    case MTP3_T7:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T7  TIMER msg.\n"));
    ...
    case MTP3_T8:
        MTP3_DEBUG(("MTP3_Main: handle T8  TIMER msg.\n"));
        MTP3_HDR_SET_SIO(mtp3, ctxt.data.routeInfo.sio);
        MTP3_RL_SET_DPC_VALUE(mtp3.label, ctxt.data.routeInfo.pointCode);
    ...
    case MTP3_T10:
        MTP3_DEBUG(("MTP3_Main: handle T10  TIMER msg.\n"));
        MTP3_HDR_SET_SIO(mtp3, ctxt.data.routeInfo.sio);
        MTP3_RL_SET_DPC_VALUE(mtp3.label, ctxt.data.routeInfo.pointCode);
    ...
    case MTP3_T11:
        MTP3_DEBUG(("MTP3_Main: handle T11  TIMER msg.\n"));
        MTP3_HDR_SET_SIO(mtp3, ctxt.data.routeInfo.sio);
        MTP3_RL_SET_DPC_VALUE(mtp3.label, ctxt.data.routeInfo.pointCode);
    ...
    case MTP3_T12:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T12  TIMER msg.\n"));
    ...
    case MTP3_T13:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T13  TIMER msg.\n"));
    ...
    case MTP3_T14:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T14  TIMER msg.\n"));
    ...
    case MTP3_T15:
        MTP3_DEBUG(("MTP3_Main: handle T15  TIMER msg.\n"));
        ...
        ret = TRCC_Main_CCITT(MTP3_TIMER, STM_TRIGGER_T11,
                              ev.data, ev.len,
                              ctxt.data.routeInfo.linkSet,
                              ITS_SS7_DEFAULT_LINK_SET);
    ...
    case MTP3_T16:
        MTP3_DEBUG(("MTP3_Main: handle T16  TIMER msg.\n"));
        MTP3_HDR_SET_SIO(mtp3, ctxt.data.routeInfo.sio);
        MTP3_RL_SET_DPC_VALUE(mtp3.label, ctxt.data.routeInfo.pointCode);
    ...
    case MTP3_T17:
        MTP3_ERROR(("MTP3_Main: handle T17  TIMER msg.\n"));
    ...
    //    {
    //         MTP3_ERROR(("MTP3_MainT17: Unable to lock DIDS table\n"));
   //          return (NULL);
   //     }

        ret = LSAC_Main_CCITT(MTP3_TIMER, SLM_TRIGGER_T17,
                               NULL, 0,
                               ctxt.data.linkInfo.linkSet,
                               ctxt.data.linkInfo.linkCode);
    //    DSM_UnlockTable(DSM_Mtp3FtGroupId, DSM_TABLE_ITU_DIDS); 
        return(ret);
    case MTP3_T18:
        MTP3_DEBUG(("MTP3_Main: handle T18  TIMER msg.\n"));
        MTP3_HDR_SET_SIO(mtp3, ctxt.data.routeInfo.sio);
        MTP3_RL_SET_DPC_VALUE(mtp3.label, ctxt.data.routeInfo.pointCode);
    ...
    case MTP3_T19:
        MTP3_DEBUG(("MTP3_Main: handle T19  TIMER msg.\n"));
    ...
    case MTP3_T20:
        MTP3_DEBUG(("MTP3_Main: handle T20  TIMER msg.\n"));
    ...
    case MTP3_T21:
        MTP3_DEBUG(("MTP3_Main: handle T21  TIMER msg.\n"));
    ...
    case MTP3_T22:
        /* Increament peg for Timer T18 expired */
        PEG_IncrPeg(CCITT_MTP3_Pegs,PEG_MTP3_T18_EXPIRED);
        MTP3_DEBUG(("MTP3_Main: handle T22  TIMER msg.\n"));
    ...
    case MTP3_T23:
        /* Increament peg for Timer T18 expired */
        PEG_IncrPeg(CCITT_MTP3_Pegs,PEG_MTP3_T18_EXPIRED);
        MTP3_DEBUG(("MTP3_Main: handle T23  TIMER msg.\n"));
    ...
    case MTP3_T24:
        /* Increament peg for Timer T20 expired */
        PEG_IncrPeg(CCITT_MTP3_Pegs,PEG_MTP3_T20_EXPIRED);
        MTP3_DEBUG(("MTP3_Main: handle T24  TIMER msg.\n"));
        ...
        return TPRC_Main_CCITT(MTP3_TIMER, STM_TRIGGER_T24,
                               ...
                               ctxt.data.linkInfo.linkCode);
    case MTP3_T25:
        MTP3_DEBUG(("MTP3_Main: handle T25  TIMER msg.\n"));
        MTP3_HDR_SET_SIO(mtp3, ctxt.data.routeInfo.sio);
        MTP3_RL_SET_DPC_VALUE(mtp3.label, ctxt.data.routeInfo.pointCode);
    ...
    case MTP3_T26:
        MTP3_DEBUG(("MTP3_Main: handle T26  TIMER msg.\n"));
    ...
    case MTP3_T27:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T27  TIMER msg.\n"));
    ...
    case MTP3_T28:
        MTP3_DEBUG(("MTP3_Main: handle T28  TIMER msg.\n"));
        MTP3_HDR_SET_SIO(mtp3, ctxt.data.routeInfo.sio);
        MTP3_RL_SET_DPC_VALUE(mtp3.label, ctxt.data.routeInfo.pointCode);
    ...
    case MTP3_T29:
        MTP3_DEBUG(("MTP3_Main: handle T29  TIMER msg.\n"));
        MTP3_HDR_SET_SIO(mtp3, ctxt.data.routeInfo.sio);
        MTP3_RL_SET_DPC_VALUE(mtp3.label, ctxt.data.routeInfo.pointCode);
    ...
    case MTP3_T30:
        MTP3_DEBUG(("MTP3_Main: handle T30  TIMER msg.\n"));
        MTP3_HDR_SET_SIO(mtp3, ctxt.data.routeInfo.sio);
        MTP3_RL_SET_DPC_VALUE(mtp3.label, ctxt.data.routeInfo.pointCode);
    ...
    case MTP3_T31:
         MTP3_DEBUG(("MTP3_Main: handle T31  TIMER msg.\n"));
    ...
    case MTP3_T32:
        MTP3_DEBUG(("MTP3_Main: handle T32  TIMER msg.\n"));
    ...
    case MTP3_T33:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T33  TIMER msg.\n"));
    ...
    case MTP3_T34:
        ...
        MTP3_DEBUG(("MTP3_Main: handle T34  TIMER msg.\n"));
    ...
    case MTP3_SLT_T1:
        MTP3_DEBUG(("MTP3_Main: handle MTP3_SLT_T1  TIMER msg.\n"));
    ...
    case MTP3_SLT_T2:
            MTP3_DEBUG(("MTP3_Main: handle MTP3_SLT_T2  TIMER msg.\n"));
    ...
    case MTP3_LEAVE_TMR:
            ...
            else if(NODECARDSTATE == 73)
            {
              MTP3_DEBUG(("MTP3_Main: since Node state is ACTIVE: Calling PEER_NODELEAVE\n"));
              PEER_NODELEAVEOP(&ev);
            } 
            else
            {
              MTP3_DEBUG(("MTP3_Main: since Node state is not ACTIVE & STANDBY:do nothing\n"));
            }
            break;
...
CCITT_MTP3Thread(void *arg)
{
    ...
    cont = ITS_TRUE;
    MTP3_CRITICAL(("CCITT_MTP3Thread: ID %lu .\n",(unsigned long)threadId));
//    MTP3_CRITICAL(("CCITT_MTP3Thread: ID %d .\n",syscall(207)));
           ...
           == ITS_SUCCESS)
    {
        ...
        case ITS_MTP3_SRC:
            MTP3_DEBUG(("CCITT_MTP3Thread: handle termination event.\n"));
            TRANSPORT_IS_ACTIVE(__MTP3_CCITT_RcvQueue) = ITS_FALSE;
            cont = ITS_FALSE;
            break;
        case ITS_SCCP_SRC:
        case ITS_ISUP_SRC:
             MTP3_DEBUG(("CCITT_MTP3Thread: handle L4 msg.\n"));
            ...
            MTP3_DEBUG(("LOCK ITS_SCCP_SRC ITS_ISUP_SRC  %s %d\n",__FILE__,__LINE__));
            ...
            ROUTE_UnlockRead(routeRWLock, __MTP3_CCITT_RcvQueue);
            MTP3_DEBUG(("UNLOCK ITS_SCCP_SRC ITS_ISUP_SRC  %s %d\n",__FILE__,__LINE__));
            break;
        ...
        case ITS_TIMER_SRC:
        {
            ...
            MTP3_DEBUG(("MTP3 Timer  LockForWrite xxxxxxxxxxxxxxxx %s %d\n",__FILE__,__LINE__));
            if (ROUTE_LockForWrite(routeRWLock) != ITS_SUCCESS)
            {
                MTP3_ERROR(("Failed to get route lock\n"));
            ...
            MTP3_DEBUG(("MTP3 Timer  ULockForWrite xxxxxxxxxxxxxxxx %s %d\n",__FILE__,__LINE__));
            ROUTE_UnlockWrite(routeRWLock);
            MTP3_DEBUG(("MTP3 Timer  ULockForWrite Sucess xxxxxxxxxxxxxxxx %s %d\n",__FILE__,__LINE__));
            break;
        ...
        case HEALTH_MANAGER_INSTANCE: 
             {
              ...
              {
                 MTP3_DEBUG((" MTP3 CARD is not ACTIVE & STANDBY  , so no LEAVE TIMER is started"));
              }
              MTP3_DEBUG(("MTP3_DEBUG#: SELF:thisInstanceIsReading[%d][%d]= %d \n",nodeId,threadId,routeRWLock->thisInstanceIsReading[nodeId][threadId]));
              MTP3_DEBUG(("##########################################################################\n")); 
                 ...
                 for (i = 0; i < MAX_NODES; i++) 
                 {
                     for(j = 0; j < MAX_THREADS; j++)
                     {
                         if( (i == 2) || (i == 1))
                         {
                            if(i != nodeId)
                            {
                            MTP3_DEBUG(("MTP3_Thread:ON_LEAVE:ReadFlag[%d][%d] = [%d] ",i,j,routeRWLock->thisInstanceIsReading[i][j]));
                            }
                         }
        ...
        default:
            ...
            if (MTP3_MSG_TYPE(&mtp3Event) == MTP3_MSG_USER_DATA)
            {
                ...
                MTP3_DEBUG(("LOCK MTP3_MSG_USER_DATA  %s %d\n",__FILE__,__LINE__));
                ...
                MTP3_DEBUG(("UNLOCK MTP3_MSG_USER_DATA  %s %d\n",__FILE__,__LINE__));
                ROUTE_UnlockRead(routeRWLock, __MTP3_CCITT_RcvQueue );
            }
            else
            {
                                           ...
                                           buf, &len) != ITS_SUCCESS)
                {
                    ...
                    MTP3_Alarm_CCITT(2405, __FILE__, __LINE__, NULL);
                }
                else
                {
                                    ...
                                    if (dest != ITS_INVALID_SRC && dest != ITS_NO_SRC)
                                  {
                                    ...
                                    if ((ret = MTP3_Encode(&ev, MTP3_MSG_USER_DATA, mtp3,
                                        ...
                                        (len - sizeof(MTP3_HEADER) -sizeof(ITS_OCTET)))) != ITS_SUCCESS)
                                       {
                                        ...
                                        MTP3_Alarm_CCITT(2501, __FILE__, __LINE__, "Destination %d", dest);
                                        return (ret);
                                       }
                                   }
               ...
               /*********************************************************************************************/
                    ...
                    if (NeedLock(buf))
                    {
                        ...
                        isLockSet = ITS_TRUE;
                        MTP3_DEBUG(("L3 MGMT LockForWrite xxxxxxxxxxxxxxxx %s %d\n",__FILE__,__LINE__));
                    }
                    else
                    {
                        if (ROUTE_LockForRead(routeRWLock) != ITS_SUCCESS)
                        ...
                        MTP3_DEBUG(("L4 MSG  LockForRead xxxxxxxxxxxxxxxx %s %d\n",__FILE__,__LINE__));
                    }
                    ...
                    if (isLockSet)
                    {
                        ...
                        ROUTE_UnlockWrite(routeRWLock);
                        MTP3_DEBUG(("L3 MGMT  UnlockWrite xxxxxxxxxxxxxxxx %s %d\n",__FILE__,__LINE__));
                    }
                    else
                    {
                        ROUTE_UnlockRead(routeRWLock, __MTP3_CCITT_RcvQueue);
                        MTP3_DEBUG(("L4  UnlockRead xxxxxxxxxxxxxxxx %s %d\n",__FILE__,__LINE__));
                    }
    ...
    MTP3_ERROR(("MTP3_Main: handle terminated.\n"));
    TRANSPORT_IS_ACTIVE(__MTP3_CCITT_RcvQueue) = ITS_FALSE;
    THREAD_NORMAL_EXIT;
}
...
//        MTP3_DEBUG(("DSM_FT_GROUP_MEMBER_LEAVE nodeID: %d\n",concernedNodeId));
    ...
    case DSM_FT_GROUP_MEMBER_DEAD:
	  ...
	  Trans_Ret=TRANSPORT_PutEvent(ITS_MTP3_SRC, &mtp3Event);
          if(Trans_Ret != ITS_SUCCESS)
	  {
            MTP3_ERROR((" Unable to put in MTP3 Transport Queue\n"));
            MTP3_ERROR((" So ABORTING........"));
	    abort();
	  }
          MTP3_DEBUG (("Event put to MTP3 Queue\n"));
          Trans_Ret=TRANSPORT_PutEvent (ITS_SCCP_SRC, &mtp3Event);
          if(Trans_Ret != ITS_SUCCESS)
	  {
            MTP3_ERROR((" Unable to put in SCCP Transport Queue\n"));
            MTP3_ERROR((" So ABORTING........"));
	    abort();
	  }
    ...
    case DSM_FT_GROUP_MEMBER_ALIVE:
         MTP3_DEBUG(("MTP3 DSM_FT_GROUP_MEMBER_ALIVE nodeID: %d\n",concernedNodeId));
     ...
     for (i = 0; i < lscount; i++)
		 {
						 ...
						 for(j = 0; j < MAX_LINKS_PER_LINKSET; j++)
						 {
										 ...
										 {
														 MTP3_ERROR (("MTP3LEAVE:For link ls|ls %d|%d instance id =%d isUp %d FA %d FILE:%s LINE:%d\n", linkset[i],j,ln->id,ln->isUp,ITS_FALSE, __FILE__, __LINE__));
                             if (ln->activityState == LINK_ACTIVITY_ACTIVE)
                                {
                                  status=1;
                                     ...
                                     if(ln->isUp != ITS_FALSE)
                                      { 
                                        MTP3_Alarm_CCITT(2509, __FILE__, __LINE__, "linkset:linkcode %d:%d",linkset[i],j);
																				MTP3_ERROR(("MTP3LEAVE:Alarm Gen ls|lc %d|%d\n",linkset[i],j));
                                      }
...
NODESTATUS(ITS_EVENT *evt)
{
    ...
    ITS_INT LINK_REM_FLAG = ITS_FALSE;
     ...
     for (k = 0; k < MAX_LINKSETS; k++)
      ...
      MTP3_ERROR(("MTP3 NODESTATUS: No of LinkSet =%d",lscount));
     ...
     for (i = 0; i < lscount; i++)
		 {
						 ...
						 for(j = 0; j < MAX_LINKS_PER_LINKSET; j++)
						 {
										 if((ln = LINK_FindLink ((ITS_OCTET)linkset[i], j)) != NULL)
										 {
														 MTP3_ERROR (("MTP3 NODESTATUS  ls|ln %d|%d instance id =%d isUp %d FA %d FILE:%s LINE:%d\n", linkset[i],j,ln->id,ln->isUp,ITS_FALSE, __FILE__, __LINE__));
										 }
						 }
...
static int MarkAllRoutesUnAvailable(ITS_POINTER entry, void *in, void *out)
{
    ...
    ITS_UINT i;
    ITS_UINT dpc=*((ITS_UINT*)in);
    MTP3_ERROR(("MarkAllRoutesUnAvailable  dpc %d dest->status %d",dpc,dest->status));
    MTP3_ERROR(("MarkAllRoutesUnAvailable NumEntries  %d \n",dest->numEntries));

    for(i = 0; i < dest->numEntries; i++)
    {
        MTP3_ERROR(("For family(1) %d|basic.type(2) %d| basic.style(0) %d | adjacent(22) %d |basic.dpc %d\n",dest->family,dest->entries[i]->basic.type,dest->entries[i]->basic.style,dest->entries[i]->linkSet->adjacent,dest->entries[i]->basic.dpc));
        if ( dest->entries[i]->basic.type == REMOTE_ROUTE && (dest->entries[i]->basic.style == ROUTE_DPC_NI ) &&
             (dest->entries[i]->linkSet->adjacent != 0) &&(dpc == dest->entries[i]->basic.dpc) )
        {
            MTP3_ERROR(("MarkAllRoutesUnAvailable::First if Sucess Value of i %d \n",i));
            dest->entries[i]->isAllowed = ITS_FALSE;
            dest->entries[i]->isRestricted = ITS_FALSE;
            dest->status = DESTINATION_UNAVAILABLE;
            MTP3_ERROR(("MarkAllRoutesUnAvailable  DESTINATION_UNAVAILABLE PC %d dest->status %d",dest->entries[i]->basic.dpc,dest->status));
       }
   }
    return (ITS_SUCCESS);
}
