...
ROUTE_Info * ROUTE_FindDest(ITS_UINT pointCode, ITS_OCTET ni, SS7_Family family);
...
//    if (Sccp_Info->PCFlag==ITS_FALSE||IsGtt==ITS_FALSE)
    ...
    length+=sizeof(ITS_OCTET)+sizeof(ITS_CTXT)+sizeof(MTP3_HEADER);
	  ...
	  ITS_EVENT_INIT(&event,ITS_SCCP_SRC,length*sizeof(ITS_OCTET));
		  ...
		  ITS_OCTET Ssn)
{
  ...
  SCCP_DEBUG (("DPC->%d OPC->%d ni->%d SSN->ssn\n", DPC, OPC, ni, Ssn));
  destPtr = ROUTE_FindDest (OPC, ni, FAMILY_ITU);
  if (destPtr == NULL)
    {
      *pauseFlag = ITS_TRUE;
      *sspFlag = ITS_TRUE;
  ...
  for (rInfo = destPtr; rInfo != NULL; rInfo = rInfo->next)
    {
      ...
      if (rInfo->basic.dpc == DPC && rInfo->basic.criteria.ssn == Ssn
      	  && rInfo->basic.style == ROUTE_DPC_OPC_SIO_SSN)
     	{
      	  ...
      	  ret = DSM_Commit (DSM_SccpFtGroupId, DSM_TABLE_ITU_SCCP_LPC,
  ...
  dest = ROUTE_FindDestination (DPC, ni, FAMILY_ITU);
      ...
      if (rInfo->basic.dpc == DPC && rInfo->basic.criteria.ssn == Ssn)
      	{
          	  ...
          	  SCCP_CRITICAL (("SCCP_SetConfig: Key=%d\n", key));
           	  ret = DSM_Commit (DSM_SccpFtGroupId, DSM_TABLE_ITU_SCCP_LPC, (ITS_OCTET *) & key, sizeof (ITS_UINT), destPtr);
          	  if (ret != ITS_SUCCESS)
              {
