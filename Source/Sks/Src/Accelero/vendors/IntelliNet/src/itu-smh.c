 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-smh.c,v 1.1.1.1 2007-10-08 11:12:15 bsccs2 Exp $
 *
 ...
 * LOG: update from MSC
 * LOG:
 * LOG: Revision 1.3  2007/04/17 11:26:59  ashutosh.singh
 * LOG: Logger lib changes
 * LOG:
 * LOG: Revision 1.2  2007/04/17 08:23:37  ashutosh.singh
...
#ident "$Id: itu-smh.c,v 1.1.1.1 2007-10-08 11:12:15 bsccs2 Exp $"
               ...
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    case SMH_TRIGGER_L2_L3_MSG:
        ...
        default:
            ...
            MTP3_Alarm_CCITT(2406, __FILE__, __LINE__, "%d from L2", sif[0]);
               ...
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
             ...
             MTP3_NIC_RESERVED)
    {
        ...
        else
        {
            ...
            MTP3_Alarm_CCITT(2407, __FILE__, __LINE__, NULL); 
                                          ...
                                          ITS_SS7_DEFAULT_SLS)) != ITS_SUCCESS)
    {
        ...
        MTP3_Alarm_CCITT(2408, __FILE__, __LINE__, "user part %d",up);
    ...
    {
        MTP3_DEBUG(("Sending Msg Not Meant for Our  Point Code Length->%d\n",len));
                  ...
                  ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    rinfo.rkey = 0;
    MTP3_DEBUG(("MessageForRouting: sls %d from SCCP Routing lable linkSet linkCode %d|%d\n",sls,linkSet,linkCode));
...
//    sls = (sls << 1) | ((sls >> 3) & 1);
     ...
     */
     if (ROUTE_GetRouteAdminStatus(dpc, &isUse,
                                        ROUTE_DPC_NI,
                                        FAMILY_ITU,
                                        ni,
                                        ITS_SS7_DEFAULT_LINK_SET,
                                        ...
                                        ITS_SS7_DEFAULT_SLS) != ITS_SUCCESS)
        {
          MTP3_DEBUG(("MessageForRouting: AdminState Wrong Param\n"));
   ...
   if(isUse == ITS_FALSE)
   {
         MTP3_DEBUG(("MessageForRouting: AdminState Lock\n"));
         return (!ITS_SUCCESS); 

   }
    MTP3_DEBUG(("Before MessageForRouting: sls %d rinfo ls|lc  %d|%d\n",sls,rinfo.linkSet, rinfo.linkCode));
    ...
    //MTP3_DEBUG(("After MessageForRouting: sls %d rinfo ls|lc  %d|%d\n",sls,rinfo.linkSet, rinfo.linkCode));
    ...
    if (dest == ITS_NO_SRC || dest == ITS_INVALID_SRC)
    {
        ...
        else
        {
            if ((rc = ROUTE_FindDestination(dpc, ni, FAMILY_ITU)) == NULL)
            {
                ...
                MTP3_Alarm_CCITT(2411, __FILE__, __LINE__, "dpc %d ni %d", 
                                 dpc, ni);
            ...
            if (rc->status == DESTINATION_UNAVAILABLE)
            {
                ...
                MTP3_Alarm_CCITT(2412, __FILE__, __LINE__,"dpc %d ni %d LinkSet %d",dpc, ni, linkSet);       
    ...
    else
    {
        if ((rc = ROUTE_FindDestination(dpc, ni, FAMILY_ITU)) == NULL)
        {
            ...
            MTP3_Alarm_CCITT(2411, __FILE__, __LINE__, "dpc %d ni %d", dpc, ni);
        ...
        if (rc->status == DESTINATION_RESTRICTED)
        {
            ...
            MTP3_Alarm_CCITT(2413, __FILE__, __LINE__, "dpc %d ni %d",dpc, ni);
                      ...
                      ITS_OCTET linkSet, ITS_OCTET linkCode)
{
  ...
  //  sls <<= 1;
     if (ROUTE_GetRouteAdminStatus(dpc, &isUse,
                                        ...
                                        ITS_SS7_DEFAULT_SLS) != ITS_SUCCESS)
        {
          MTP3_DEBUG(("LinkManagement: AdminState Wrong Param\n"));
   ...
   if(isUse == ITS_FALSE)
   {
         MTP3_DEBUG(("LinkManagement: AdminState Lock\n"));
         return (!ITS_SUCCESS); 
                                      ...
                                      sls) != ITS_SUCCESS)
    {
        MTP3_Alarm_CCITT(2502, __FILE__, __LINE__,
                         "destination %d,linkset:linkcode %d:%d",
    ...
    if (dest == ITS_NO_SRC || dest == ITS_INVALID_SRC)
    {
        MTP3_Alarm_CCITT(2414, __FILE__, __LINE__, "dpc %d ni %d",dpc,ni);
        /* discard if it isn't */
        MTP3_Alarm_CCITT(2502, __FILE__, __LINE__,
                         "destination %d,linkset:linkcode %d:%d",
                         dpc,linkSet,linkCode);
                         ...
                         ITS_OCTET linkSet, ITS_OCTET linkCode)
{
   ...
   // sls <<= 1;
     if (ROUTE_GetRouteAdminStatus(dpc, &isUse,
                                        ...
                                        ITS_SS7_DEFAULT_SLS) != ITS_SUCCESS)
        {
          MTP3_DEBUG(("TrafficMamagement: AdminState Wrong Param\n"));
   ...
   if(isUse == ITS_FALSE)
   {
         MTP3_DEBUG(("TrafficManagement: AdminState Lock\n"));
         return (!ITS_SUCCESS); 
    ...
    if (dest == ITS_NO_SRC || dest == ITS_INVALID_SRC)
    {
        MTP3_Alarm_CCITT(2415, __FILE__, __LINE__, "dpc %d ni %d LinkSet %d",dpc,ni,rinfo.linkSet);
    ...
    else if (SNMM_HC0_GET(*snmm) == SNMM_HC0_ECM)
    {
        ...
        {
            MTP3_Alarm_CCITT(2515, __FILE__, __LINE__, "ECO Trasmitted on %d:%d",
                            linkSet,linkCode);
                   ...
                   ITS_OCTET linkSet, ITS_OCTET linkCode)
{
   ...
   // sls <<= 1;
     ...
     if (ROUTE_GetRouteAdminStatus(dpc, &isUse,
                                        ...
                                        ITS_SS7_DEFAULT_SLS) != ITS_SUCCESS)
        {
            MTP3_DEBUG(("TestControMessage : AdminState Wrong Param\n"));
            return (ITS_EINVALIDARGS);
        }
   ...
   if(isUse == ITS_FALSE)
   {
         MTP3_DEBUG(("TestControlMessage: AdminState Lock\n"));
         return (!ITS_SUCCESS); 
    ...
    if (dest == ITS_NO_SRC || dest == ITS_INVALID_SRC)
    {
        MTP3_Alarm_CCITT(2416, __FILE__, __LINE__, "dpc %d ni %d linkSet %d",dpc,ni,linkSet);
               ...
               ITS_UINT whichEntry)
{
    ...
    if ((rc = ROUTE_FindDestination(dpc, ni, FAMILY_ITU)) == NULL)
    {
        MTP3_ERROR(("FindDestination failed, return.\n"));

        MTP3_Alarm_CCITT(2419, __FILE__, __LINE__, "dpc %d ni %d",dpc,ni);
                   ...
                   ITS_OCTET *data, ITS_USHORT len)
{
    ...
    if ((ln = LINK_FindLinkNoLock(linkSet, linkCode)) == NULL)
    {
        ...
        MTP3_Alarm_CCITT(2420, __FILE__, __LINE__, "The Link is %d:%d", 
                         linkSet, linkCode);
...
*/
        ...
        if (!MUST_PASS(data) && ln->qPut)
        {
            ...
            evRec.event = ev;
            MTP3_ERROR((" Putting Msg in cocbBuf Buffer 1967\n"));
        ...
        int ret;
         ...
         if(ln->id == 20002)
         {
           if(ln->isUp ==-1 || ln->isUp == 0)
           {
            ...
            {
             MTP3_DEBUG(("L3_L2_RETRIEVE_BSNT \n"));
               ...
               if(PEER_MTP3_NODE_STATUS == 0)
                 ...
                 {
                   ret = TRANSPORT_PutEvent(ITS_MTP3_SRC, &fail);
                   return ITS_SUCCESS; 
                 }
