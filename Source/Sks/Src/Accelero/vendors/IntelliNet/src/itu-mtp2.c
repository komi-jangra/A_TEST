...
/***************** To Print the Data Received on MTP2 Channel****/
  ...
  for (count = 0; count < len; count++)	/*only 100 bytes will print */
	{
	  sprintf ((char *) TempBuffer, "%x ", data[count]);
	  strcat ((char *) buff, (const char *) TempBuffer);
  ...
  case SCM_NWSTK_LINK_ADMIN_STATE_MODIFY:
      {
        ...
        mtp2Config.Pe  =mtp2timer->mtp2EmrgncyProvngPriod;
        mtp2Config.Pn  =mtp2timer->mtp2NormalProvngPriod;
  ...
  if (!mtp2Inited)
						     ...
						     5)) == NULL)
	{
      ...
      if ((__MTP2_CCITT_RcvQueue =
	   TQUEUETRAN_CreateTransport ("ITU-MTP2", ITS_MTP2_CCITT_SRC,
				       ITS_MTP2_CCITT)) == NULL)
	{
