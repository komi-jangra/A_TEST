 ...
 * LOG: update from MSC
 * LOG:
 * LOG: Revision 1.3  2007/07/25 11:27:11  cvsadmin
 * LOG: *** empty log message ***
 * LOG:
 * LOG: Revision 1.2  2007/05/11 06:14:03  ashutosh.singh
 * LOG: change for logger lib integeration
 * LOG:
 * LOG: Revision 1.1  2007/01/06 06:54:57  cvsadmin
 * LOG: *** empty log message ***
 * LOG:
...
#ident "$Id: itu-mtp2sim.c,v 1.6 2008-03-14 04:42:11 bsccs2 Exp $"
...
/*  data[0]= (data[0] & 0x0f);
  data[0]= data[0] | 0x40;*/
   MTP2_DEBUG(("ReadDataFromDev:MTP2SIM   FD = %d len= %d \n",ctxt->dlink[idx].fd,ret)); 

#else
   MTP2_DEBUG(("ReadDataFromDev:MTP2SIM   FD = %d  \n",ctxt->dlink[idx].fd)); 
...
CCITT_MTP2_RcvThread(void *args)
{
    ...
    ITS_INT         millSecs = 10000;
       ...
       MTP2_DEBUG(("********** CCITT_MTP2_RcvThread *************\n"));
    while (!isSetExitMTP2)
    {
...
MTP2_DEBUG(("CCITT_MTP2_SendThread :\n"));
...
for (; i < ITS_SS7_CTXT_NUMLINKS(ctxt); i++)
{
	...
	remote_ipaddr =0; 
       ...
       if(connected_flag[i] ==0)
       {
	     ...
	     MTP2_DEBUG(("localIpAddr= %s, local_port= %d\n",localIpAddr,local_port[i])); 	

	     MTP2_DEBUG(("localIpAddr= %x, local_port= %x\n",me.sin_addr.s_addr, me.sin_port)); 	

              if (bind( sockfd, (struct sockaddr *) &me, sizeof(me) ) < 0)
              {
              MTP2_ERROR(("Unable to Bind (client)"));
              close(sockfd);
		connected_flag[i] =0;
		sockfd =-1;
              goto  Next;
            ...
            if ((remote_ipaddr = inet_addr (remoteIpAddr[i])) == -1)
             {
                MTP2_ERROR(("Invalid remote IP address\n"));
                close(sockfd);
		connected_flag[i] =0;
		sockfd =-1;
                goto  Next;
	    ...
	    MTP2_DEBUG(("RemoteIpAddr= %s, Remote_port= %d\n",remoteIpAddr,remote_port[i])); 	

	    MTP2_DEBUG(("RemoteIpAddr= %x, Remote_port= %x\n",saddr.sin_addr.s_addr, saddr.sin_port)); 	
...
Next:	  	
   ...
   MTP2_DEBUG((" MTP2SIM MTP2Bind end \n"));
    return ITS_SUCCESS;
...
MTP2_DEBUG((" MTP2Bind IN  NOT_USING_UTILn"));
        mtp2Cfg.num_chan = 1;
...
IMTP2_Initialize_CCITT(ITS_SS7_HANDLE handle)
{
    ...
    if (!mtp2Inited)
    {
        ...
        {
            MTP2_ERROR(("IMTP2_Initialize: Could not create trace Data.\n"));
            return ITS_ENOMEM;
        }
 ...
 for(loop =0; loop<len; loop++)
  {
        ...
        while (nleft > 0)
        {
                ...
                if (nwritten < 0)
                 {
		   MTP2_ERROR(("error  in byte sending on socket\n  "));
		   break;	
		}		
			//        return nwritten;        /* error return < 0 */
		MTP2_DEBUG(("left %d", nleft));
                nleft -= nwritten;
                data+= nwritten;
        }
        /*return (nbytes - nleft);*/

   	MTP2_DEBUG((" MTP2SIM  SendDataToDev:: send len = %d >>\n", nleft));
	ret =len-nleft;
#else
    ...
    if (found)
    {
        ...
        switch (rxdata.signal)
        {
            ...
            case L3_L2_GET_CONFIG:
                ...
                break;
            case L3_L2_GET_PEGS:
                MTP2_DEBUG(("L3_L2_GET_PEGS:LS=%d:LC=%d\n",
                            linkSet, linkCode));
                sigValid = ITS_FALSE;
                break;
