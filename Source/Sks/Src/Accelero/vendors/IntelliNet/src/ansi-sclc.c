 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-sclc.c,v 1.1.1.1 2007-10-08 11:12:13 bsccs2 Exp $
 *
...
#ident "$Id: ansi-sclc.c,v 1.1.1.1 2007-10-08 11:12:13 bsccs2 Exp $"
...
#define MSG_LEN  (272 - 5 - 3 - 8 -3 + 4 - 3 )/*MTP3 HDR - MFixed - ptrtoParam - param len field + seq control -mtp2 header*/
                              ...
                              ITS_OCTET reason)
{
    ...
    case SCCP_MSG_LUDT:
        ...
        return SCRCHandleSCLCMsg_ANSI(&resp, SCCP_MSG_LUDTS, lies, 5);
                       ...
                       ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    case SCCP_MSG_LUDT:
        PEG_IncrPeg(ANSI_SCCP_Pegs, PEG_SCCP_REMOTE_LUDT_RCVD);

        if (SCCP_Encode(ies, ieCount, mType, &event, &hdr,
                        &SCCP_LUDT_Desc_ANSI) != ITS_SUCCESS)
        {
            SCCP_ERROR(("SCLCHandleSCRCMsg: Encode failure\n"));
    ...
    case SCCP_MSG_LUDTS:
        PEG_IncrPeg(ANSI_SCCP_Pegs, PEG_SCCP_REMOTE_LUDTS_RCVD);

        if (SCCP_Encode(ies, ieCount, mType, &event, &hdr,
                        &SCCP_LUDTS_Desc_ANSI) != ITS_SUCCESS)
        {
            SCCP_ERROR(("SCLCHandleSCRCMsg: Encode failure\n"));
                       ...
                       ITS_SCCP_IE* ies, int ieCount)
{
    ...
    case SCCP_MSG_LUDT:
        return SCRCHandleSCLCMsg_ANSI(mtp3, SCCP_MSG_LUDT, ies, ieCount);

    case SCCP_MSG_LUDTS:
        return SCRCHandleSCLCMsg_ANSI(mtp3, SCCP_MSG_LUDTS, ies, ieCount);
...
SCLCHandleAPPMsg_ANSI(ITS_EVENT* event)
{
    ...
    case SCCP_MSG_UDT:
      ...
      //possible only in case of Protocol Class=0 otherwise from APP
        ...
        if (mtp3 == NULL)
        {
            ...
            SCCP_Alarm_ANSI(633, __FILE__, __LINE__, "UDT");
    ...
    case SCCP_MSG_XUDT:
        ...
        {
            /* ERROR - Discarding %1 Message - Defective MTP3 header.  */
            SCCP_Alarm_ANSI(633, __FILE__, __LINE__, "XUDT");
    ...
    case SCCP_MSG_UDTS:
        ...
        {
            /* ERROR - Discarding %1 Message - Defective MTP3 header.  */
            SCCP_Alarm_ANSI(633, __FILE__, __LINE__, "UDTS");
    ...
    case SCCP_MSG_XUDTS:
        ...
        {
            /* ERROR - Discarding %1 Message - Defective MTP3 header.  */
            SCCP_Alarm_ANSI(633, __FILE__, __LINE__, "XUDTS");
    ...
    case SCCP_MSG_LUDT:
        PEG_IncrPeg(ANSI_SCCP_Pegs, PEG_SCCP_USER_LUDT_SENT);
        ...
        if (mtp3 == NULL)
        {
            ...
            SCCP_Alarm_ANSI(633, __FILE__, __LINE__, "LUDT");
        ...
        for (i = 0; i < ieCount; i++)
        {
            ...
            {
                    CDA_LEN=ies[i].param_length;              
            }
           else if(ies[i].param_id ==SCCP_PRM_CALLING_PARTY_ADDR)
            {
                    CPA_LEN=ies[i].param_length;              
            
            }
        }
        SCCP_DEBUG(("SCLCHandleAPPMsg:CDA_LEN %d CPA_LEN %d\n", CDA_LEN,CPA_LEN));
        for (i = 0; i < ieCount; i++)
        {
           ...
           else if (ies[i].param_id ==SCCP_PRM_CALLED_PARTY_ADDR)
            {
                ...
                if(cdp->data[0] & SCCP_CPA_ROUTE_SSN) 
                 ...
                 { 
                   datavalue= (MSG_LEN - CDA_LEN - CPA_LEN);
                   allowedseg= (datavalue - 10);
                   SCCP_DEBUG(("SCLCHandleAPPMsg: GTT are Present MSG_LEN %d  datavalue %d allowedseg %d\n",MSG_LEN,datavalue,allowedseg));
                 }
 ...
 //              totalSegment = dataLen / SCCP_MAX_SEG_DATA;
           ...
           // if (dataLen % SCCP_MAX_SEG_DATA)
            ...
            if (totalSegment > SCCP_MAX_ALLOWED_SEGMENT)
            {
                ...
                PEG_IncrPeg(ANSI_SCCP_Pegs, PEG_SCCP_USER_SEGMENTATION_ERR);
            ...
            for (i = 0;i < totalSegment; i++)
            {
                ...
                ret = SCRCHandleSCLCMsg_ANSI(mtp3, SCCP_MSG_XUDT, ies, ieCount);
        ...
        {
            ret = SCRCHandleSCLCMsg_ANSI(mtp3, SCCP_MSG_LUDT, ies, ieCount);
    ...
    case SCCP_MSG_LUDTS:
        PEG_IncrPeg(ANSI_SCCP_Pegs,   PEG_SCCP_USER_LUDTS_SENT);
        ...
        if (mtp3 == NULL)
        {
            ...
            /* Incoming Message - Discarding  Message - Defective MTP3 header. */
            SCCP_Alarm_ANSI(633, __FILE__, __LINE__, "LUDTS");
