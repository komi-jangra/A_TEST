...
//LOG_PRINT(INFO,"Prot_handle_bcast Function called \n");
  ...
  //LOG_PRINT(INFO,"received_bytes == 0\n");
	...
	{
   LOG_PRINT(INFO,"status Msg Recieved\n");
	...
	{
   LOG_PRINT(INFO,"FC  Msg Recieved\n");
        ...
        {
    						LOG_PRINT(INFO,"Conf Reload Msg Recievd  Msg Recieved\n");
	...
	{
   LOG_PRINT(INFO,"Memmber ship message received \n");
		Memb_handle_message( &New_pack );
		return;
	}
	if( Is_hurry( pack_ptr->type ) )
	{
  // LOG_PRINT(INFO,"Hurry type MSG message received \n");
	...
	if( !Is_regular( pack_ptr->type ) )
	{
		...
		LOG_PRINT(CRITICAL,"PROTOCOL Prot_handle_bcast: Unknown packet type %d\n", pack_ptr->type );
		return;
	}
	if( ! Memb_is_equal( Memb_id(), pack_ptr->memb_id ) )
	{
   LOG_PRINT(INFO,"Foreign message\n");
		/* Foreign message */
		Memb_handle_message( &New_pack );
		return;
	}
	...
	received_bytes -= sizeof(packet_header);
     ...
     //LOG_PRINT(INFO,"num_buffered_packets =%d\n",num_buffered_packets);
...
void	Prot_handle_token(channel fd, int dummy, void *dummy_p)
{
	...
	int		i, ret;
  struct timespec req,rem,tmp_rem;
  int delay=0;
/*********************for milli sec sleep **************************/
static  struct timeval    select_delay = { 0, 100 };
  fd_set      readfds;

//LOG_PRINT(INFO,"Prot_handle_token function called\n"); 
/****************************end here *****************************/
	/*	int		r1,r2;*/
	...
	if( ret == 0 ) 
    {
 // LOG_PRINT(INFO,"received_bytes on token channel == 0\n");
     return;
   }
	...
	{
      //         LOG_PRINT(INFO,"PROTOCOL, \"it is a Form Token.\n");
	...
	if( ! Memb_token_alive() ) {
       //        LOG_PRINT(INFO,"PROTOCOL, \"Prot_handle_token: Veto Property. Memb not alive.\n");
	...
	if( Conf_leader( Memb_active_ptr() ) == My.id )
	{
	//	LOG_PRINT(INFO,"Conf_leader True\n");
		...
		if( Get_arq(Token->type) == Get_arq(Last_token->type) )
		{
//		LOG_PRINT(INFO,"Token type is same \n");
		    if( Get_retrans(Token->type) > Get_retrans(Last_token->type) )
		    {
//		LOG_PRINT(INFO,"Token retrans type is same Get_retrans(Token->type) > Get_retrans(Last_token->type  \n");
			...
			Alarm( PROTOCOL, "Prot_handle_token: not leader, swallow same token %d %d\n", Get_arq(Token->type), Get_retrans(Token->type) );
	//		LOG_PRINT(INFO,PROTOCOL, "Prot_handle_token: not leader, swallow same token %d %d\n", Get_arq(Token->type), Get_retrans(Token->type) );
		    }
		    return;
  ...
  //LOG_PRINT(INFO,"\nProt_handle_token function called and the Highest_seq=%d My_aru=%d\n",Highest_seq,My_aru);
  ...
  //LOG_PRINT(INFO,"num_allowed=%d num_sent=%d\n",num_allowed,num_sent);
  ...
  //LOG_PRINT(INFO,"\n AFTER FOR LOOP Prot_handle_token function called and the Highest_seq=%d My_aru=%d\n",Highest_seq,My_aru);
	GlobalStatus.my_aru = My_aru;
	...
	if( ( Token->aru == Set_aru    ) || ( Token->aru_last_id == My.id ) || ( Token->aru == Token->seq ) )
	{
		Token->aru = My_aru;
	...
	/* Send token */
	if( ! ( Conf_leader( Memb_active_ptr() ) == My.id && To_hold_token() ) )
	{
  //   for(delay=0;delay < 900000;delay++);
    /*******to delay for token as we care less for latency****/
//  	if(Conf_leader( Memb_active_ptr() ) == My.id)	
     //select(0,0,0,0,&select_delay);
   //select(1,NULL,NULL,NULL,&tmp_time);
    //usleep(1);
   // req.tv_sec=0;
    //req.tv_nsec=5555 ;
 ...
 //   {
  ///  req.tv_sec=0;
   ...
   // nanosleep(&req,&rem);
    //}
     //  nanosleep(&rem,&tmp_rem);
    //LOG_PRINT(INFO,"\nkilled after nanosleep\n");
   /******** delay chages ends here****************************/
		/* sending token */
...
{
	int32	leader_id;
  static unsigned long int nmsg_cnt=0;
   ...
   nmsg_cnt++;
 //LOG_PRINT(INFO,"(num_mess > 0)  %d nmsg_cnt=%d\n",Down_queue_ptr->num_mess,nmsg_cnt); 
 ...
 //LOG_PRINT(INFO,"(num_mess == 0) %d nmsg_cnt=%d\n",Down_queue_ptr->num_mess,nmsg_cnt); 
	...
	{
    //LOG_PRINT(INFO,"__FILE__ %s __LINE__ %d\n",__FILE__,__LINE__);
...
static	int	Send_new_packets( int num_allowed )
{
	...
	num_sent = 0;
		...
		scat_ptr = Down_queue_ptr->first->mess;
                ...
                Highest_seq++;
    //LOG_PRINT(INFO,"Send_new_packets Function is called with Highest_seq++=%d\n",Highest_seq);
		...
		if( Down_queue_ptr->cur_element < scat_ptr->num_elements )
		{
			...
			Alarm( EXIT, 
			  ...
			  Down_queue_ptr->cur_element,scat_ptr->num_elements );
				...
				Alarm( PROTOCOL, "type|transmiter_id|proc_id|memb_id|seq|fifo_seq|packet_index|data_len|\n");
				Alarm( PROTOCOL, "%x|%x|%x|%x|%x|%x|%x|%d|\n",pack_ptr->type,pack_ptr->transmiter_id,pack_ptr->proc_id,pack_ptr->memb_id,pack_ptr->seq,pack_ptr->fifo_seq,pack_ptr->packet_index,pack_ptr->data_len);
...
{	
	...
	if( up_ptr->mess->num_elements+1 != index )
	{
		Alarm( EXIT, "Deliver_packet: sequence error: sec is %d, should be %d\n",
	         pack_ptr->packet_index,	up_ptr->mess->num_elements+1 );
	}
...
static	void	Deliver_reliable_packets( int32 start_seq, int num_packets )
{
	...
	for( i = start_seq; i <= end_seq  ; i++ )
	{
        ...
        j++;
			...
			//	LOG_PRINT(INFO," PROTOCOL Deliver_reliable_packets: packet %d was delivered\n", j );
}
	...
	for( i = Last_delivered+1; i <= My_aru; i++ )
	{
        ...
        j++;
			...
			Last_delivered++;
				...
				LOG_PRINT(INFO,"PROTOCOL, Deliver_agreed_packets: packet %d was delivered\n", j );
        if(j>15 && j<20)
				LOG_PRINT(INFO,"RANGE 15-20 ************** Agreed packet %d  delivered****************\n", j );
        if(j>20)
				LOG_PRINT(INFO,"RANGE Greater THAN 20 ######### Agreed packet %d  delivered############\n", j );
}
...
void	Prot_token_hurry()
{
	...
	int32		val;
// LOG_PRINT(INFO,"Prot_token_hurry()\n");
	...
	//LOG_PRINT(INFO,"PROTOCOL, Prot_token_hurry: retransmiting token %d %d\n", Get_arq(Last_token->type), Get_retrans(Last_token->type) );
