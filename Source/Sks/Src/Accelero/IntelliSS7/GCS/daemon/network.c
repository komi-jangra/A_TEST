...
void	Net_init()
{
	...
	if( Cn->segments[My.seg_index].num_procs > 1 )
	{
        ...
        for ( i=0; i < My.num_if; i++)
        {
           ...
           LOG_PRINT(INFO,"Bcast_channel => %d \n",Bcast_channel[Num_bcast_channels]);
                        Token_channel[Num_token_channels++] = DL_init_channel( RECV_CHANNEL, My.port+1, 0, interface_addr );
           LOG_PRINT(INFO,"Token_channel => %d \n",Token_channel[Num_token_channels]);
                }
        }

	Send_channel  = DL_init_channel( SEND_CHANNEL, My.port+2, 0, My.id );
 LOG_PRINT(INFO,"Send channel %d \n",Send_channel);
...
int	Net_bcast( sys_scatter *scat )
{
 ...
 LOG_PRINT(INFO,"\nNum_send_needed =%d Send_channel %d  Send_address[%d]=%d Send_ports[%d]=%d\n",Num_send_needed,Send_channel,i,Send_address[i],i,Send_ports[i]);
LOG_PRINT(INFO,"Number of bytes send =%d\n",ret);
	}
...
//       #########Printing the Token Header ########
   Alarm(NETWORK,"NSToken Head As follows|type|transmiter_id|seq|proc_id|aru|aru_last_id|flow_control|rtr_len|conf_hash\n");
   Alarm(NETWORK,"NSToken Head As follows|%x|%x|%x|%x|%x|%x|%x|%x|%x\n",token_ptr->type,token_ptr->transmiter_id,token_ptr->seq,token_ptr->proc_id,token_ptr->aru,token_ptr->aru_last_id,token_ptr->flow_control,token_ptr->rtr_len,token_ptr->conf_hash);
...
int	Net_recv_token( channel fd, sys_scatter *scat )
{
	...
	token_ptr = (token_header *)scat->elements[0].buf;
   Alarm(NETWORK,"NRToken Head As follows|type|transmiter_id|seq|proc_id|aru|aru_last_id|flow_control|rtr_len|conf_hash\n");
   Alarm(NETWORK,"NRToken Head As follows|%x|%x|%x|%x|%x|%x|%x|%x|%x\n",token_ptr->type,token_ptr->transmiter_id,token_ptr->seq,token_ptr->proc_id,token_ptr->aru,token_ptr->aru_last_id,token_ptr->flow_control,token_ptr->rtr_len,token_ptr->conf_hash);
        ch_found = FALSE;
  ...
  if(ret == 0)
  {
  LOG_PRINT(INFO,"\n received byte equal to zero on tok \n");
  }
	if( ret <= 0 ) return( ret );
