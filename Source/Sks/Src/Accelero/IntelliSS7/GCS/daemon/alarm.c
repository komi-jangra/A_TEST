...
void Alarmp( int16 priority, int32 mask, char *message, ...)
{
	...
	{
	   //LOG_PRINT(CRITICAL,"Exit caused by Alarm(EXIT)\n");
            exit( 0 );
	}
}
                        ...
                        void *ptr17, void *ptr18, void *ptr19, void *ptr20)
{
	  ...
	  // LOG_PRINT(INFO,message, ptr1, ptr2, ptr3, ptr4, ptr5, ptr6, ptr7, ptr8, ptr9, ptr10, ptr11, ptr12, ptr13, ptr14, ptr15, ptr16, ptr17, ptr18, ptr19, ptr20 );
...
void Alarm_set_output(char *filename) {
        ...
        if ( NULL == newfile ) {
              // LOG_PRINT(CRITICAL,"failed to open file (%s) for stdout. Error: %d\n", filename, errno);
        }
        newfile = freopen(filename, "a", stderr);
        if ( NULL == newfile ) {
              // LOG_PRINT(CRITICAL,"failed to open file (%s) for stderr. Error: %d\n", filename, errno);
        }
