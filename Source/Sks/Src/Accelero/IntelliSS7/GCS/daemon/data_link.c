...
channel	DL_init_channel( int32 channel_type, int16 port, int32 mcast_address, int32 interface_address )
{
	...
	{
        	if (setsockopt(chan, SOL_SOCKET, SO_BROADCAST, (char *)&on, sizeof(on)) < 0) 
		...
		ttl_val = 1;
        	if (setsockopt(chan, IPPROTO_IP, IP_MULTICAST_TTL, (void *)&ttl_val, sizeof(ttl_val)) < 0) 
		{
	...
	if ( channel_type & RECV_CHANNEL )
	{
		...
		if( i1 >=224 && i1 < 240 )
		{
			...
			mreq.imr_interface.s_addr = INADDR_ANY;
        		...
        		if (setsockopt(chan, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&mreq, sizeof(mreq)) < 0) 
			{
        perror("setsockopt(IP_ADD_MEMBERSHIP)"); 
        printf("setsockopt(IP_ADD_MEMBERSHIP)%s",strerror(errno)); 
...
//  for(k=0;k<10;k++)
  //      {
    //    Alarm( DATA_LINK, "DL_send: buf[%d]  \t: %x \n", k, scat->elements[0].buf[k]);
      //  }
	return(ret);
}
...
static	char		pseudo_scat[MAX_PACKET_SIZE];
 ...
 //printf("DL_recv function ret=%d\n",ret);
  ...
  //      for(k=0;k<10;k++)
    //    {
      //  Alarm( DATA_LINK, "DL_rec: buf[%d]  \t: %x \n", k, scat->elements[0].buf[k]);
        //}
