...
void	Conf_load_conf_file( char *file_name, char *my_name )
{
	...
	if (yyin == NULL) 
		if (NULL != (yyin = fopen("./gcs_daemon.in", "r")) )
                        Alarm( PRINT, "Conf_load_conf_file: using file: ./gcs_daemon.in\n");
  if (yyin == NULL)
    if (NULL != (yyin = fopen("/etc/gcs_daemon.ini", "r")) )
                        Alarm( PRINT, "Conf_init: using file: /etc/gcs_daemon.ini\n");
	if (yyin == NULL)
