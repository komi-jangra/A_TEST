...
#endif	/* ARCH_PC_WIN95 */
LOG_PRINT(INFO,"Sess_attach_accept functon called Accept_unix_mbox%d\n",Accept_unix_mbox);
...
static	void	Sess_attach_accept()
{
        ...
        {
								printf("Accept_inet_mbox[%d]=>%d\n",i,Accept_inet_mbox[i]);
...
static	void	Sess_accept( mailbox mbox, int domain, void *dummy )
{
   ...
   LOG_PRINT(INFO,"Sess_accept for AF_INET = %d\n",Sessions[MAX_SESSIONS].mbox);
   ...
   LOG_PRINT(INFO,"Sess_accept for AF_UNIX  = %d\n",Sessions[MAX_SESSIONS].mbox);
	...
	if( Num_sessions == MAX_SESSIONS )
	{
        ...
        if ( ( (i = Sess_get_session_index(Sessions[MAX_SESSIONS].mbox)) != -1)&&(Sessions[i].mbox == Sessions[MAX_SESSIONS].mbox)&&(Is_op_session( Sessions[i].status )) )
        {
...
void    Sess_session_authorized(int ses)
{
        ...
        E_attach_fd( Sessions[ses].mbox, READ_FD, Sess_read, Sessions[ses].type, NULL, LOW_PRIORITY );
        E_attach_fd( Sessions[ses].mbox, EXCEPT_FD, Sess_read, Sessions[ses].type, NULL, LOW_PRIORITY );
