...
void ip_init(void)
{
    ...
    /* load gcs_daemon_ip_access.ini file */
    if (NULL != (fp = fopen(file_name,"r")) )
        Alarmp( SPLOG_INFO, ACM, "ip_init: using file: %s\n", file_name);
    if (fp == NULL) 
        if (NULL != (fp = fopen("./gcs_daemon_ip_access.ini", "r")) )
            Alarmp( SPLOG_INFO, ACM, "ip_init: using file: ./gcs_daemon_ip_access.ini\n");
    if (fp == NULL)
        if (NULL != (fp = fopen("/etc/spread.access_ip", "r")) )
            Alarmp( SPLOG_INFO, ACM, "ip_init: using file: /etc/gcs_daemon_ip_access.ini\n");
    if (fp == NULL)
    {
