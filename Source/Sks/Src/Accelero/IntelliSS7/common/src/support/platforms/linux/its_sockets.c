 ...
 * LOG: Revision 9.1.8.1  2006/04/17 16:56:15  yranade
...
#ident "$Id: its_sockets.c,v 1.1.1.1 2006-12-22 13:34:48 cvsadmin Exp $"
                ...
                const char *host_or_path, int port) 
{
    ...
    /*Note::: To add for KEEPALIVE OPTION FOR Accepted scoket START:-*/
    socklen_t optlen;
    int optval = 0;
    /*Note::: To add for KEEPALIVE OPTION FOR Accepted scoket ENDS:-*/
    ...
    *sockInfo = NULL;
    ...
    if (domain == INET_DOMAIN)
    {
       ...
       /*Note::: To add for KEEPALIVE OPTION FOR Accepted scoket START:-*/
         ...
         if(setsockopt(ret->socket, SOL_SOCKET, SO_KEEPALIVE, &optval, optlen) < 0) 
           {
            perror("setsockopt()");
            close(ret->socket);
           } 
         ...
         optval = 2;
         optlen = sizeof(optval);
         if(setsockopt(ret->socket, SOL_TCP, TCP_KEEPCNT, &optval, optlen) < 0)
           {
            perror("setsockopt()");
            close(ret->socket);
           }
         ...
         optval = 5;
         optlen = sizeof(optval);
         if(setsockopt(ret->socket, SOL_TCP, TCP_KEEPIDLE, &optval, optlen) < 0)
           {
            perror("setsockopt()");
            close(ret->socket);
           }
         ...
         optval = 2;
         optlen = sizeof(optval);
         if(setsockopt(ret->socket, SOL_TCP, TCP_KEEPINTVL, &optval, optlen) < 0)  
           {
             perror("setsockopt()");
             close(ret->socket);
           }
                ...
                const char *host_or_path, int port) 
{
    ...
    if (domain == INET_DOMAIN)
    {
       if (bind(ret->socket, (struct sockaddr *)&saddr, ret->asize) < 0) 
       {
           free(ret->addr);
           close(ret->socket);
           free(ret);
...
SOCK_ServerAccept(ITS_SocketInfo* info, ITS_SocketInfo **newInfo)
{
    ...
    *newInfo = NULL;
    ...
    /*Note::: To add for KEEPALIVE OPTION FOR Accepted scoket START:-*/
     optval = 1;
     optlen = sizeof(optval);
     if(setsockopt(tmp->socket, SOL_SOCKET, SO_KEEPALIVE, &optval, optlen) < 0) 
       {
        perror("setsockopt()");
        close(tmp->socket);
       } 
      ITS_TRACE_ERROR(("SOCK_ServerAccept: SO_KEEPALIVE set on socket\n"));

     optval = 2;
     optlen = sizeof(optval);
     if(setsockopt(tmp->socket, SOL_TCP, TCP_KEEPCNT, &optval, optlen) < 0)
       {
        perror("setsockopt()");
        close(tmp->socket);
       }
      ITS_TRACE_ERROR(("SOCK_ServerAccept: SO_KEEPALIVE->TCP_KEEPCNT set on socket\n"));

    optval = 5;
    optlen = sizeof(optval);
    if(setsockopt(tmp->socket, SOL_TCP, TCP_KEEPIDLE, &optval, optlen) < 0)
      {
        perror("setsockopt()");
        close(tmp->socket);
      }
      ITS_TRACE_ERROR(("SOCK_ServerAccept: TCP_KEEPCNT -> TCP_KEEPIDLE set on socket\n"));
        
    optval = 2;
    optlen = sizeof(optval);
    if(setsockopt(tmp->socket, SOL_TCP, TCP_KEEPINTVL, &optval, optlen) < 0)  
      {
       perror("setsockopt()");
       close(tmp->socket);
      }
      ITS_TRACE_ERROR(("SOCK_ServerAccept: TCP_KEEPIDLE -> TCP_KEEPINTVL set on socket\n"));
