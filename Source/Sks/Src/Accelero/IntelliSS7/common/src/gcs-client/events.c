...
int 	E_init(void)
{
	...
	Time_queue = NULL;
        ...
        if (ret < 0)
        {
                ...
                ITS_TRACE_ERROR(("E_Init: failed to initialize TIME_EVENT memory objects\n"));

                /* HLD: added return here. */
...
sp_time	E_get_time(void)
{
	...
	if ( ret < 0 )
    {
        Alarm( EXIT, "E_get_time: gettimeofday problems.\n" );
        ITS_TRACE_ERROR(("E_get_time: gettimeofday failed\n"));
    }
...
sp_time	E_sub_time( sp_time t, sp_time delta_t )
{
	...
	if ( res.sec < 0 ) 
    {
        Alarm( EVENTS, "E_sub_time: negative time result.\n");
        ITS_TRACE_DEBUG(("E_sub_time: negative time result\n"));
    }
	return ( res );
}
		 ...
		 sp_time delta_time )
{
	...
	if( Time_queue != NULL )
	{
                    ...
                    Time_queue->code == t_e->code )
		{
            ...
            ITS_TRACE_DEBUG(("E_queue: dequeued a (first) simillar event\n"));
		}
	}
	if( Time_queue == NULL )
	{
        ...
        ITS_TRACE_DEBUG(("E_queue: (only) event queued func 0x%x code %d data 0x%x in future (%u:%u)\n",t_e->func,t_e->code, t_e->data, delta_time.sec, delta_time.usec ));
		return( 0 );
	}else{
		...
		if( compare < 0 )
		{
            ...
            ITS_TRACE_DEBUG(("E_queue: (first) event queued func 0x%x code %d data 0x%x in future (%u:%u)\n",t_e->func,t_e->code, t_e->data, delta_time.sec,delta_time.usec ));
		}
	}
	t_pre    = Time_queue ; 
	...
	while ( t_post != NULL && ( !inserted || !deleted ) )
	{
                    ...
                    t_post->code == t_e->code )
		{
            ...
            ITS_TRACE_DEBUG(("E_queue: dequeued a similar event\n"));
			continue;
		}
		...
		if ( !inserted )
		{
			...
			if( compare < 0 )
			{
                ...
                ITS_TRACE_DEBUG(("E_queue: event queued for func 0x%x code %d data 0x%x in future (%u:%u)\n",t_e->func,t_e->code, t_e->data, delta_time.sec, delta_time.usec));
			}
		}
	...
	if( !inserted )
	{
        ...
        ITS_TRACE_DEBUG(("E_queue: (last) event queued func 0x%x code %d data 0x%x in future (%u:%u)\n",t_e->func,t_e->code, t_e->data, delta_time.sec,delta_time.usec ));
	}
		   ...
		   void *data )
{
	...
	if( Time_queue == NULL )
	{
        ...
        ITS_TRACE_ERROR(("E_dequeue: no such event\n"));
		return( -1 );
	}
            ...
            Time_queue->code == code )
	{
        ...
        ITS_TRACE_DEBUG(("E_dequeue: first event dequeued func 0x%x code %d data 0x%x\n",func,code, data));
		return( 0 );
	}
	...
	while ( t_pre->next != NULL )
	{
                    ...
                    t_ptr->code == code )   
		{
            ...
            ITS_TRACE_DEBUG(("E_dequeue: event dequeued func 0x%x code %d data 0x%x\n",func,code, data));
			return( 0 );
		}
		t_pre = t_ptr;
	}
...
void	E_delay( sp_time t )
{
	...
	tmp_t = t;
        if (select(0, NULL, NULL, NULL, (struct timeval *)&tmp_t ) < 0)
        {
                ...
                ITS_TRACE_ERROR(("E_delay: select delay returned error: %s\n", strerror(errno)));
        }
		     ...
		     int code, void *data, int priority )
{
	...
	if( priority < 0 || priority > NUM_PRIORITY )
	{
        ...
        ITS_TRACE_ERROR(("E_attach_fd: invalid priority %d for fd %d with fd_type %d\n", priority, fd, fd_type ));
		return( -1 );
	}
	if( fd_type < 0 || fd_type > NUM_FDTYPES )
	{
		Alarm( PRINT, "E_attach_fd: invalid fd_type %d for fd %d with priority %d\n", fd_type, fd, priority );
        ITS_TRACE_ERROR(("E_attach_fd: invalid fd_type %d for fd %d with priority %d\n", fd_type, fd, priority ));
		return( -1 );
	}
#ifndef	ARCH_PC_WIN95
	 ...
	 */
        if( fd < 0 || fd > FD_SETSIZE )
        {
                ...
                ITS_TRACE_ERROR(("E_attach_fd: invalid fd %d (max %d) with fd_type %d with priority %d\n", fd, FD_SETSIZE, fd_type, priority ));
                return( -1 );
        }
#endif
	for( j=0; j < Fd_queue[priority].num_fds; j++ )
	{
		if( ( Fd_queue[priority].events[j].fd == fd ) && ( Fd_queue[priority].events[j].fd_type == fd_type ) )
		{
			...
			Alarm( PRINT, "E_attach_fd: fd %d with type %d exists & replaced & activated\n", fd, fd_type );
            ITS_TRACE_DEBUG(("E_attach_fd: fd %d with type %d exists and replaced and activated\n", fd, fd_type ));
			return( 1 );
		}
	}
	num_fds = Fd_queue[priority].num_fds;
        ...
        if ( num_fds == MAX_FD_EVENTS ) {
                ...
                ITS_TRACE_ERROR(("E_attach_fd: reached Maximum number of events - recompile with larger MAX_FD_EVENTS\n"));
                return( -1 );
        }
    ...
    ITS_TRACE_DEBUG(("E_attach_fd: fd %d, fd_type %d, code %d, data 0x%x, priority %d Active_priority %d\n", fd, fd_type, code, data, priority, Active_priority ));
...
int 	E_detach_fd( int fd, int fd_type )
{
	...
	if( fd_type < 0 || fd_type > NUM_FDTYPES )
	{
        ...
        ITS_TRACE_ERROR(("E_detach_fd: invalid fd_type %d for fd %d\n", fd_type, fd ));
		return( -1 );
	}
...
int     E_deactivate_fd( int fd, int fd_type )
{
	...
	if( fd_type < 0 || fd_type > NUM_FDTYPES )
	{
        ...
        ITS_TRACE_ERROR(("E_deactivate_fd: invalid fd_type %d for fd %d\n", fd_type, fd ));
		return( -1 );
	}
...
int     E_activate_fd( int fd, int fd_type )
{
	...
	if( fd_type < 0 || fd_type > NUM_FDTYPES )
	{
        ...
        ITS_TRACE_ERROR(("E_activate_fd: invalid fd_type %d for fd %d\n", fd_type, fd ));
		return( -1 );
	}
...
int 	E_set_active_threshold( int priority )
{
	...
	if( priority < 0 || priority > NUM_PRIORITY )
	{
        ...
        ITS_TRACE_ERROR(("E_set_active_threshold: invalid priority %d\n", priority ));
		return( -1 );
	}
    ...
    ITS_TRACE_DEBUG(("E_set_active_threshold: changed to %d\n",Active_priority));
...
int	E_num_active( int priority )
{
	if( priority < 0 || priority > NUM_PRIORITY )
	{
        ...
        ITS_TRACE_ERROR(("E_num_active: invalid priority %d\n", priority ));
		return( -1 );
	}
...
static	const sp_time		mili_sec 	= {     0, 1000};
    ...
    ITS_TRACE_DEBUG(("E_handle_events: next event\n"));
	...
	while( Time_queue != NULL )
	{
		...
		if ( !first && E_compare_time( Now, Time_queue->t ) >= 0 )
		{
            ...
            ITS_TRACE_DEBUG(("E_handle_events: exec time event\n"));
#ifdef TESTTIME 
                        Alarm( TR_DEBUG, "Events: TimeEv is %d %d late\n",tmp_late.sec, tmp_late.usec); 
                        ITS_TRACE_DEBUG(("E_handle_events: time event is %d %d late\n",tmp_late.sec, tmp_late.usec)); 
#endif
        ...
        Alarm(TR_DEBUG, "Events: TimeEv's took %d %d to handle\n", tmp_late.sec, tmp_late.usec); 
        ITS_TRACE_DEBUG(("E_handle_events: time event took %d %d to handle\n", tmp_late.sec, tmp_late.usec)); 
#endif
	/* Handle fd events   */
	for( i=0; i < NUM_FDTYPES; i++ )
	{
    ...
    ITS_TRACE_DEBUG(("E_handle_events: ready to poll select\n"));
#ifdef TESTTIME
        req_time = zero_sec;
#endif
	...
	if (num_set == 0 && !Exit_events)
	{
        ...
        ITS_TRACE_DEBUG(("E_handle_events: select with timeout (%d, %d)\n", timeout.sec,timeout.usec ));
#ifdef TESTTIME
        ...
        Alarm( TR_DEBUG, "Events: Waiting for fd or timout took %d %d asked for %d %d\n", tmp_late.sec, tmp_late.usec, req_time.sec, req_time.usec);
        ITS_TRACE_DEBUG(("E_handle_events: waiting for fd or timout took %d %d asked for %d %d\n", tmp_late.sec, tmp_late.usec, req_time.sec, req_time.usec));
#endif
	     ...
	     i-- )
	{
		...
		if( FD_ISSET( fd, &current_mask[fd_type] ) )
		{
            ...
            ITS_TRACE_DEBUG(("E_handle_events: exec handler for fd %d, fd_type %d, priority %d\n", fd, fd_type, i ));
        ...
        stop = E_get_time();
        tmp_late = E_sub_time(stop, start);
        Alarm(TR_DEBUG, "Events: High & Med took %d %d time to handle\n", tmp_late.sec, tmp_late.usec);
        ITS_TRACE_DEBUG(("E_handle_events: high and medium time events took %d %d time to handle\n", tmp_late.sec, tmp_late.usec));
#endif
             ...
             i++ )
	{
        ...
        ITS_TRACE_DEBUG(("E_handle_events: exec ext fd event \n"));
        ...
        start = E_get_time();
        tmp_late = E_sub_time(start, stop);
        Alarm(TR_DEBUG, "Events: Low priority took %d %d to handle\n", tmp_late.sec, tmp_late.usec);
        ITS_TRACE_DEBUG(("E_handle_events: low priority time events took %d %d to handle\n", tmp_late.sec, tmp_late.usec));
#endif
    }
 end_handler:
