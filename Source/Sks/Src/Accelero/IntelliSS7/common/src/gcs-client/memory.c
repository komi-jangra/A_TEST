...
void    Mem_init_status()
{
        ...
        if (!MemStatus_initialized) 
        {
                ...
                if (!sgroup)
                {
                        ...
                        ITS_TRACE_ERROR(("Mem_init_object: failed to create Memory Status group\n"));
        ...
        for (obj_type = 1; obj_type < MAX_MEM_OBJECTS; obj_type++)
        {
                if (Mem[obj_type].exist) {
                        ...
                        if (!sgroup)
                                ...
                                ITS_TRACE_ERROR(("Mem_init_object: failed to create statistics group for object: %s\n", Objnum_to_String(obj_type)));
                        else {
#ifndef NDEBUG
...
void            Mem_init_object_abort( int32u obj_type, int32u size, unsigned int threshold, unsigned int initial )
{
        ...
        if (ret < 0 ) {
                ...
                ITS_TRACE_ERROR(("Mem_init_object_abort: failed to initialize an object: %s\n", obj_name));
        }
}
        ...
        if (initial > 0)
        {
                ...
                for(i = initial; i > 0; i--)
                {
                        ...
                        if (head_ptr == NULL) 
                        {
                                ...
                                ITS_TRACE_ERROR(("Mem_init_object: out of memory - returning with existing buffers\n"));
                                mem_error = 1;
                                break;
                        }
...
void *          new(int32u obj_type)
{
        ...
        if (Mem[obj_type].list_head == NULL) 
        {
                ...
                if (head_ptr == NULL) 
                {
                        ...
                        ITS_TRACE_ERROR(("Mem_alloc_object: out of memory - returning NULL object\n"));
                        return(NULL);
                }
                head_ptr->obj_type = obj_type;
                head_ptr->block_len = sizeobj(obj_type);
...
void *          Mem_alloc( unsigned int length)
{
        ...
        if (head_ptr == NULL) 
        {
                ...
                ITS_TRACE_ERROR(("Mem_alloc: out of memory - returning NULL block\n"));
                return(NULL);
        }
...
void            dispose(void *object)
{
        ...
        ITS_TRACE_DEBUG(("Dispose: disposing pointer 0x%x to object type %d named %s\n", object, obj_type, Objnum_to_String(obj_type)));
