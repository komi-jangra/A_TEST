...
struct acp_ops {
       ...
       ACM_ReturnVal (*p2p_send) (char *user, char dests[][MAX_GROUP_NAME], int service_type);
       ACM_ReturnVal (*mcast_send) (char *user, char groups[][MAX_GROUP_NAME], int service_type);
};
...
struct session_auth_info *Acm_auth_create_sess_info_forIP(int ses);
struct session_auth_info *Acm_auth_create_sess_info(int ses, char *auth_name);
