...
/* HLD: added ITS_TRACE as duplication of Alarm. */
#ifndef WIN32
...
static  void    sp_initialize_locks(void)
{
	...
	if( ret == 0 )
	{
		...
		if( ret != 0 )
        {
            ...
            ITS_TRACE_ERROR(("SP_initialize_locks: winsock initialization error %d\n", ret ));
        }
#endif	/* ARCH_PC_WIN95 */
	}
        return;
...
int     SP_set_auth_method( const char *auth_name, int (*auth_function) (int, void *), void * auth_data)
{
        ...
        if (strlen(auth_name) >= MAX_AUTH_NAME)
        {
            ...
            ITS_TRACE_ERROR(("SP_set_auth_method: name of auth method too long\n"));
            return(0);
        }
        if ( NULL == auth_function )
        {
            Alarm( SESSION, "SP_set_auth_method: auth method is NULL\n");
            ITS_TRACE_ERROR(("SP_set_auth_method: auth method is NULL\n"));
            return(0);
        }
...
int     SP_set_auth_methods( int num_methods, const char *auth_name[], int (*auth_function[]) (int, void *), void * auth_data[])
{
        ...
        if (num_methods < 0 || num_methods > MAX_AUTH_METHODS)
        {
            ...
            ITS_TRACE_ERROR(("SP_set_auth_methods: trying to register too many methods\n"));
            return(0);
        }
...
int	SPStub_connect_timeout( const char *spread_name, const char *private_name,
		    int priority, int group_membership, mailbox *mbox,
		    char *private_group, sp_time time_out )
{
...
#ifdef ENABLEDEBUG
        Alarm_set(SESSION | TR_DEBUG);
#endif

        /* HLD: added stack variables memset. */
        memset(auth_list, 0, MAX_AUTH_NAME * MAX_AUTH_METHODS);
	...
	if( p == AF_INET )
	{
		...
		if( s < 0 )
		{
			Alarm( TR_DEBUG, "StubSP_connect: unable to create mailbox %d\n", s );
            //ITS_TRACE_ERROR(("StubSP_connect: unable to create mailbox %d\n", s ));
...
/*	### Bug in Linux 2.0.0 - inet STREAM sockets 
	...
	if( i > 30 ) break; */
		    ...
		    if( on < i*1024 ) break;
		    Alarm( SESSION, "StubSP_connect: set sndbuf %d, ret is %d\n", on, ret );
            ITS_TRACE_DEBUG(("StubSP_connect: set sndbuf %d, ret is %d\n", on, ret ));

		    onlen = sizeof(on);
		    ret= getsockopt( s, SOL_SOCKET, SO_RCVBUF, (void *)&on, &onlen );
		    if( on < i*1024 ) break;
		    Alarm( SESSION, "StubSP_connect: set rcvbuf %d, ret is %d\n", on, ret );
            ITS_TRACE_DEBUG(("StubSP_connect: set rcvbuf %d, ret is %d\n", on, ret ));
		}
		Alarm( SESSION, "StubSP_connect: set sndbuf/rcvbuf to %d\n", 1024*(i-5) );
        ITS_TRACE_DEBUG(("StubSP_connect: set sndbuf/rcvbuf to %d\n", 1024*(i-5) ));
                ...
                if (ret < 0) 
                {
                        ...
                        //ITS_TRACE_ERROR(("StubSP_connect: setting TCP_NODELAY failed with  error: %s\n", strerror(errno)));
                }
                else 
                {
                        Alarm( SESSION, "StubSP_connect: set TCP_NODELAY for socket %d\n", s);
                        ITS_TRACE_DEBUG(("StubSP_connect: set TCP_NODELAY for socket %d\n", s));
                }
		...
		while( ((ret = connect( s, (struct sockaddr *)&inet_addr, sizeof(inet_addr) ) ) == -1) 
			...
			sockopt_len_t   elen;
                        ...
                        if (err < 0)
                        {
                                Alarm( SESSION, "StubSP_connect: connect interrupted and error in select wait: %s\n", strerror(errno));
                                //ITS_TRACE_ERROR(("StubSP_connect: connect interrupted and error in select wait: %s\n", strerror(errno)));
                                ret = -1;
                                break;
                        }
                        if (FD_ISSET(s, &rset) || FD_ISSET( s, &wset))
                        {
                                ...
                                Alarm( SESSION, "StubSP_connect: connect interrupted--but socket not selected ret= %d: %s\n", err, strerror(errno));
                                //ITS_TRACE_ERROR(("StubSP_connect: connect interrupted but socket not selected ret: %d: %s\n", err, strerror(errno)));
                                ret = -1;
                                break;
                        }
...
/*
		for( i=10; i <= 200; i+=5 )
		{
            ...
            ITS_TRACE_DEBUG(("SP_connect: set sndbuf %d, ret is %d\n", on, ret ));

		    onlen = sizeof(on);
		    ret= getsockopt( s, SOL_SOCKET, SO_RCVBUF, (void *)&on, &onlen );
		    if( on < i*1024 ) break;
		    Alarm( SESSION, "SP_connect: set rcvbuf %d, ret is %d\n", on, ret );
            ITS_TRACE_DEBUG(("SP_connect: set rcvbuf %d, ret is %d\n", on, ret ));
		}
		Alarm( SESSION, "SP_connect: set sndbuf/rcvbuf to %d\n", 1024*(i-5) );
        ITS_TRACE_DEBUG(("SP_connect: set sndbuf/rcvbuf to %d\n", 1024*(i-5) ));*/
		...
		while( ((ret = connect( s, (struct sockaddr *)&unix_addr, sizeof(unix_addr) )) == -1)
			...
			sockopt_len_t   elen;
                        ...
                        if (err < 0)
                        {
                                Alarm( SESSION, "StubSP_connect: uniz connect interrupted and error in select wait: %s\n", strerror(errno));
                                //ITS_TRACE_ERROR(("StubSP_connect: unix connect interrupted and error in select wait: %s\n", strerror(errno)));
                                ret = -1;
                                break;
                        }
                        if (FD_ISSET(s, &rset) || FD_ISSET( s, &wset))
                        {
                                ...
                                Alarm( SESSION, "StubSP_connect: unix connect interrupted--but socket not selected ret %d: %s\n", err, strerror(errno));
                                //ITS_TRACE_ERROR(("StubSP_connect: unix connect interrupted but socket not selected ret %d: %s\n", err, strerror(errno)));
                                ret = -1;
                                break;
                        }
                } /* while error case for connect */
	...
	if( ret < 0 )
	{
		Alarm( SESSION, "StubSP_connect: unable to connect mailbox %d: %s\n", s, strerror(errno));
        //ITS_TRACE_ERROR(("StubSP_connect: unable to connect mailbox %d: %s\n", s, strerror(errno)));
		close( s );
		return( COULD_NOT_CONNECT );
	}
	/* 
  ...
  //printf("StubSP_Connect    Sessions[Num_sessions].mbox %d s %d Num_sessions %d ",Sessions[Num_sessions].mbox,s,Num_sessions);
		    ...
		    char *private_group, sp_time time_out )
{
	...
	if( p == AF_INET )
	{
		...
		if( s < 0 )
		{
			Alarm( TR_DEBUG, "SP_connect: unable to create mailbox %d\n", s );
            ITS_TRACE_ERROR(("SP_connect: unable to create mailbox %d\n", s ));
			return( COULD_NOT_CONNECT );
		}
...
/*	### Bug in Linux 2.0.0 - inet STREAM sockets 
	...
	if( i > 30 ) break; */
        ...
        ITS_TRACE_DEBUG(("SP_connect: set sndbuf/rcvbuf to %d\n", 1024*(i-5) ));
                ...
                {
                        Alarm(PRINT, "Setting TCP_NODELAY failed with  error: %s\n", strerror(errno));
                        ITS_TRACE_ERROR(("SP_connect: setting TCP_NODELAY failed with  error: %s\n", strerror(errno)));
                }
                else 
                {
                        Alarm( SESSION, "SP_connect: set TCP_NODELAY for socket %d\n", s);
                        ITS_TRACE_DEBUG(("SP_connect: set TCP_NODELAY for socket %d\n", s));
                }
		...
		while( ((ret = connect( s, (struct sockaddr *)&inet_addr, sizeof(inet_addr) ) ) == -1) 
			...
			sockopt_len_t   elen;
                        ...
                        if (err < 0)
                        {
                                ...
                                ITS_TRACE_ERROR(("SP_connect: connect interrupted and error in select wait: %s\n", strerror(errno)));
                                ret = -1;
                                break;
                        }
                        if (FD_ISSET(s, &rset) || FD_ISSET( s, &wset))
                        {
                                ...
                                ITS_TRACE_ERROR(("SP_connect: connect interrupted but socket not selected ret: %d: %s\n", err, strerror(errno)));
                                ret = -1;
                                break;
                        }
                } /* while error case for connect */
		...
		while( ((ret = connect( s, (struct sockaddr *)&unix_addr, sizeof(unix_addr) )) == -1)
			...
			sockopt_len_t   elen;
                        ...
                        if (err < 0)
                        {
                                ...
                                ITS_TRACE_ERROR(("SP_connect: unix connect interrupted and error in select wait: %s\n", strerror(errno)));
                                ret = -1;
                                break;
                        }
                        if (FD_ISSET(s, &rset) || FD_ISSET( s, &wset))
                        {
                                ...
                                ITS_TRACE_ERROR(("SP_connect: unix connect interrupted but socket not selected ret %d: %s\n", err, strerror(errno)));
                                ret = -1;
                                break;
                        }
                } /* while error case for connect */
	...
	if( ret < 0 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to connect mailbox %d: %s\n", s, strerror(errno)));
		close( s );
		return( COULD_NOT_CONNECT );
	}
	/* 
	...
	if( ret != len+5 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to send name %d %d: %s\n", ret, len+5, strerror(errno)));
		close( s );
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to read auth_list_len %d: %s\n", ret, strerror(errno)));
		close( s );
		return( CONNECTION_CLOSED );
        }
        ...
        ITS_TRACE_ERROR(("SP_connect: illegal value in auth_list_len %d: %s\n", auth_list_len, strerror(errno)));
		close( s );
		return( CONNECTION_CLOSED );
        } 
        if ( auth_list_len < 0 ) 
        {
            ...
            ITS_TRACE_ERROR(("SP_connect: connection invalid with code %d while reading auth_list_len\n", auth_list_len));
            close( s );
        ...
        if ( auth_list_len != 0 )
        {
                ...
                if ( ret <= 0 )
                {
                        ...
                        ITS_TRACE_ERROR(("SP_connect: unable to read auth_list %d: %s\n", ret, strerror(errno)));
                        close( s );
                        return( CONNECTION_CLOSED );
                }

                Alarm( SESSION, "SP_connect: DEBUG: Auth list is: %s\n", auth_list);
                ITS_TRACE_DEBUG(("SP_connect: auth list is: %s\n", auth_list));
        } else {
                Alarm( SESSION, "SP_connect: DEBUG: Auth list is empty\n");
                ITS_TRACE_DEBUG(("SP_connect: auth list is empty\n"));
        }
        ...
        for (i=0; i < num_auth_methods; i++)
        {
                if ( !valid_auth_method(auth_methods[i].name, auth_list, auth_list_len) )
                {
                        ...
                        ITS_TRACE_ERROR(("SP_connect: chosen authentication method is not permitted by daemon\n"));
                        close( s );
                        return( REJECT_AUTH );
                }
        }
	...
	if( ret != (MAX_AUTH_NAME * MAX_AUTH_METHODS) )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to send auth_name %d %d: %s\n", ret, MAX_AUTH_NAME * MAX_AUTH_METHODS, strerror(errno)));
		close( s );
		return( CONNECTION_CLOSED );
	}
        ...
        for (i = 0; i< num_auth_methods; i++)
        {
                ...
                if (!ret)
                {
                        ...
                        ITS_TRACE_ERROR(("SP_connect: authentication of connection failed in method %s\n", auth_methods[i].name));
                        failed = TRUE;
                }
        }
        if ( failed )
	...
	if( ret <= 0 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to read answer %d: %s\n", ret, strerror(errno)));
		close( s );
		return( CONNECTION_CLOSED );
	}
	if( l != ACCEPT_SESSION )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: session rejected %d\n", l));
		close( s );
		return( l );
	}
        ret = recv_nointr_timeout( s, &l, 1, 0, &time_out);
	...
	if( ret <= 0 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to read version %d: %s\n", ret, strerror(errno)));
		close( s );
		return( CONNECTION_CLOSED );
	}
	sp_v1 = l;
	...
	if( ret <= 0 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to read subversion %d: %s\n", ret, strerror(errno)));
		close( s );
		return( CONNECTION_CLOSED );
	}
	sp_v2 = l;
	...
	if( ret <= 0 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to read patch version %d: %s\n", ret, strerror(errno)));
		close( s );
		return( CONNECTION_CLOSED );
	}
	sp_v3 = l;
	...
	if( sp_v1*10000 + sp_v2*100 + sp_v3 < 30100 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: old GCS version %d.%d.%d not suppoted\n", sp_v1, sp_v2, sp_v3 ));
		close( s );
	...
	if( (sp_v1*10000 + sp_v2*100 + sp_v3 < 30800) && priority > 0 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: old GCS version %d.%d.%d does not support priority other than 0\n", sp_v1, sp_v2, sp_v3 ));
		close( s );
		return( REJECT_VERSION );
	}
	...
	if( ret <= 0 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to read size of group %d: %s\n", ret, strerror(errno)));
                close( s );
		return( CONNECTION_CLOSED );
	}
	len = l;
	...
	if( ret <= 0 )
	{
        ...
        ITS_TRACE_ERROR(("SP_connect: unable to read private group %d:  %s\n", ret, strerror(errno)));
		close( s );
		return( CONNECTION_CLOSED );
	}
	private_group[len]=0;
	Alarm( TR_DEBUG, "SP_connect: connected with private group(%d bytes): %s\n", ret, private_group );
    ITS_TRACE_DEBUG(("SP_connect: connected with private group(%d bytes): %s\n", ret, private_group ));
	*mbox = s;
				       ...
				       const scatter *scat_mess )
{
	...
	int		i;
	int		ret;
/* code addition start from here */
  struct timespec req;/* added anand to reduce gcs cpu utilization*/
/*
  req.tv_sec=0;
  req.tv_nsec=1000;
...
/* end here */
	...
	for( i=0, mess_len=0; i < scat_mess->num_elements; i++ )
	{
        ...
        if ( (mess_len + num_groups * MAX_GROUP_NAME) > (MAX_SCATTER_ELEMENTS * 1440 ) )
        {
	...
	if( ret <=0 )
	{
        ...
        ITS_TRACE_ERROR(("SP_internal_multicast: error %d sending header and groups on mailbox %d: %s \n", ret, mbox, strerror(errno)));
	...
	for( len=0, i=0; i < scat_mess->num_elements; len+=ret, i++ )
	{
		...
		if( ret < 0 )
		{
            ...
            ITS_TRACE_ERROR(("SP_internal_multicast: error %d sending message data on mailbox %d: %s \n", ret, mbox, strerror(errno)));
			Mutex_unlock( &Mbox_mutex[mbox&MAX_MUTEX_MASK][0] );
			SP_kill( mbox );
			return( CONNECTION_CLOSED );
		}
...
static	char		dummy_buf[10240];
        ...
        if (!This_session_message_saved) {
                ...
                for( len=0, remain = sizeof(message_header); remain > 0;  len += ret, remain -= ret )
                {
                        while(((ret = recv( mbox, &buf_ptr[len], remain, 0 )) == -1 )
                              && ((errno == EINTR) || (errno == EAGAIN)) )
                                ;
	                      Head_ptr = (message_header *)buf_ptr;
                        if( ret <=0 )
                        {
                                Alarm( SESSION, "SP_scat_receive: failed receiving header on session %d (ret: %d len: %d): %s\n", mbox, ret, len, strerror(errno) );
                                ITS_TRACE_ERROR(("SP_scat_receive: failed receiving header on session %d (ret: %d len: %d): %s\n", mbox, ret, len, strerror(errno) ));
                                Mutex_unlock( &Mbox_mutex[mbox & MAX_MUTEX_MASK][1] );
        ...
        if ( Is_reject_mess( head_ptr->type ) )
        {
                ...
                for( len=0; remain > 0; len += ret, remain -= ret )
                {
                        ...
                        if( ret <=0 )
                        {
                                ...
                                ITS_TRACE_ERROR(("SP_scat_receive: failed receiving old_type for reject on session %d, ret is %d: %s\n", mbox, ret, strerror(errno)));
                                Mutex_unlock( &Mbox_mutex[mbox & MAX_MUTEX_MASK][1] );
                                SP_kill( mbox );
                                return( CONNECTION_CLOSED );
                        }
	...
	for( len=0; remain > 0; len += ret, remain -= ret )
	{
		...
		if( ret <=0 )
		{
            ...
            ITS_TRACE_ERROR(("SP_scat_receive: failed receiving groups on session %d, ret is %d: %s\n", mbox, ret, strerror(errno)));
			Mutex_unlock( &Mbox_mutex[mbox & MAX_MUTEX_MASK][1] );
	...
	if( short_groups )
	{
		     ...
		     remain > 0; remain -= ret ) 
		{
			...
			if( ret <=0 )
			{
                ...
                ITS_TRACE_ERROR(("SP_scat_receive: failed receiving groups overflow on session %d, ret is %d: %s\n", mbox, ret, strerror(errno) ));
				Mutex_unlock( &Mbox_mutex[mbox & MAX_MUTEX_MASK][1] );
				SP_kill( mbox );
				return( CONNECTION_CLOSED );
			}
	...
	for( len=ret; remain > 0; len += ret, remain -= ret )
	{
		...
		if( ret <=0 )
		{
            ...
            ITS_TRACE_ERROR(("SP_scat_receive: failed receiving message on session %d, ret is %d: %s\n", mbox, ret, strerror(errno) ));
			Mutex_unlock( &Mbox_mutex[mbox & MAX_MUTEX_MASK][1] );
			SP_kill( mbox );
			return( CONNECTION_CLOSED );
		}else if( ret == to_read ){
	...
	*service_type = Clear_endian( head_ptr->type );
	...
	if( short_buffer )
	{
		for( remain = head_ptr->data_len - max_mess_len; remain > 0; remain -= ret ) 
		{
			...
			if( ret <=0 )
			{
                ...
                ITS_TRACE_ERROR(("SP_scat_receive: failed receiving overflow on session %d, ret is %d: %s\n", mbox, ret, strerror(errno) ));
				Mutex_unlock( &Mbox_mutex[mbox & MAX_MUTEX_MASK][1] );
				SP_kill( mbox );
				return( CONNECTION_CLOSED );
			}
	...
	Mutex_unlock( &Mbox_mutex[mbox & MAX_MUTEX_MASK][1] );
        //ITS_TRACE_ERROR((" 3.RELEASED\n"));      
         //ITS_TRACE_ERROR((" sizeof Message header[%d] \n",sizeof(message_header)));
         //ITS_TRACE_ERROR((" head_ptr->data_len[%d] \n",head_ptr->data_len));
         head_ptr->type = Clear_endian( head_ptr->type );
         //ITS_TRACE_ERROR((" head_ptr->type[%x] \n",head_ptr->type));
         head_ptr->hint = Clear_endian( head_ptr->hint );
         //ITS_TRACE_ERROR((" head_ptr->hint[%x] \n",head_ptr->hint));
         //ITS_TRACE_ERROR((" head_ptr->num_groups[%d] \n",head_ptr->num_groups));
         //ITS_TRACE_ERROR((" Messtype[%d] \n",*mess_type));
         //ITS_TRACE_ERROR((" sender[%s] \n",sender));
         //ITS_TRACE_ERROR((" num_groups[%d] \n",*num_groups));
...
static  void	SP_kill( mailbox mbox )
{
	...
	if( ses < 0 ){ 
        ...
        ITS_TRACE_ERROR(("SP_kill: killing non existent session for mailbox %d (might be ok in a threaded case)\n",mbox ));
		Mutex_unlock( &Struct_mutex );
		return;
	}
...
/* HLD: SP_error is not used in GCS Client. */
