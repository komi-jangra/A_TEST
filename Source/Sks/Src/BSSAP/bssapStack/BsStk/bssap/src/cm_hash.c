/********************************************************************20**
     ...
     Sid:      cm_hash.c@@/main/23 - Mon Mar 22 13:06:55 2010
...
static S16 cmHashFuncBCD8   (CmHashListCp *hashListCp, U8 *key, 
                                         U16 keyLen, U16 *idx);

static S16 cmHashFuncConId  (CmHashListCp *hashListCp, U8 *key, 
                                         U16 keyLen, U16 *idx);

static S16 cmHashFuncU32Mod  (CmHashListCp *hashListCp, U8 *key, 
                                         U16 keyLen, U16 *idx);

static S16 cmHashFuncString  (CmHashListCp *hashListCp, U8 *key, 
                                         U16 keyLen, U16 *idx);

static S16 cmHashFuncDefault (CmHashListCp *hashListCp, U8 *key, 
                                         U16 keyLen, U16 *idx);

static S16 cmHashFuncAnyKey  (CmHashListCp *hashListCp, U8 *key, 
                                         U16 keyLen, U16 *idx);

static S16 cmHashMatchKey (U8 *key1, U16 keyLen1, U8 *key2, U16 keyLen2);

static S16 cmListInsert   (CmListEnt *oldEntry, CmListEnt *newEntry);

static S16 cmListDelete   (CmListEnt *entry);

/* cm_hash_c_001.main_22: Fixing warnings on GCC compiler */
static S16 cmHashFuncMult24 (CmHashListCp *hashListCp, U8 *key, U16 keyLen, U16 *idx);

static S16 cmHashFuncDirIdx (CmHashListCp *hashListCp, U8 *key, U16 keyLen, U16 *idx);
...
{
   U32             sum = 0;                /* Sum of octets for hash function */
   U32             tmpKey = 0;
   U32             tmpsum = 0;
   ...
   memcpy(&tmpsum,&tmpKey,sizeof(U32));
   //sum = *((U32 *)key);
   sum = (U32)tmpsum;
...
#ifdef DEBUG_ALL
   LOG_PRINT(INFO,"DSM_UTIL:--cmHashFuncU32Mod : idx[%d]---\n",*idx);
#endif
   return(ROK);
...
void print_key(unsigned char *bufprt,int buflen)
{
  ...
  if (buflen != 0)
  {
      LOG_PRINT(INFO,"DSM_UTIL:\n###################################################\n");
      LOG_PRINT(INFO,"DSM_UTIL:PRINT Keylen[%d] : ",buflen);
...
) 
{
   ...
   if (keyLen1 != keyLen2)
      return(RFAILED);
#ifdef DEBUG_ALL
   LOG_PRINT(INFO,"DSM_UTIL: cmHashMatchKey : --------KEY1--------\n");
   print_key(key1,keyLen1);
   LOG_PRINT(INFO,"DSM_UTIL: cmHashMatchKey : --------KEY2--------\n");
   print_key(key2,keyLen1);
   LOG_PRINT(INFO,"DSM_UTIL: cmHashMatchKey : -------------------\n");
#endif
   /* compare key strings */
...
)
{
   ...
   {
#ifndef CM_MT_HASH_BIN
      if((hashListCp->hl = (CmListEnt *)malloc(nmbBins * sizeof(CmListEnt))) == NULL)
         return(RFAILED);
#else
      if((hashListCp->hl = (CmListBinEnt *)malloc(nmbBins * sizeof(CmListBinEnt))) == NULL)
         return(RFAILED);
#endif
...
)
{
   ...
   U16  *dupEntry;                  /* pointer to entry with duplicate key */
   U16 idx;                       /* index for insertion into hash list */


   /* error check on parameters */
   if ((hashListCp == NULL) || (entry == NULL) || 
       (key == NULL) || (keyLen == 0))
      return(RFAILED);
  ...
  // LOG_PRINT(INFO,"cmListInsert : entry[%p] offset[%d]\n",entry,hashListCp->offset); 
   hashListEnt = (CmHashListEnt *) (((U8 *) entry) + hashListCp->offset);
  // LOG_PRINT(INFO,"cmListInsert : hashListEnt[%p] \n",hashListEnt); 

   /* initialize hash list entry header */
   hashListEnt->list.next = NULL;
   hashListEnt->list.prev = NULL;
   hashListEnt->keyLen    = keyLen;
   hashListEnt->key       = calloc(1,keyLen);
   //LOG_PRINT(INFO,"cmListInsert : hashListEnt->key[%p] \n",hashListEnt->key); 
   memcpy(hashListEnt->key,key,keyLen);
...
#ifndef CM_MT_HASH_BIN
   hashListCp->nmbEnt++;
   //LOG_PRINT(INFO,"cmHashListInsert: inserted to hash list: hashListCp->nmbEnt[%d] \n",hashListCp->nmbEnt);
#else
   hashListCp->hl[idx].nmbEnt++;
#endif /* #ifndef CM_MT_HASH_BIN */

//   LOG_PRINT(INFO,"DSM_UTIL:cmHashListInsert: INSERT SUCCESS:hashListEnt->key[%d] :Key[%d]:hashListCp->nmbEnt[%d] \n",*hashListEnt->key,*key,hashListCp->nmbEnt);
   return(ROK);
...
)
{
   ...
   //LOG_PRINT(INFO,"----cmListDelete : entry[%p] offset[%d]\n",entry,hashListCp->offset);
   /* get pointer to hash list entry header */
   hashListEnt = (CmHashListEnt *) (((U8 *) entry) + hashListCp->offset);
   //LOG_PRINT(INFO,"----cmListDelete : hashListEnt[%p] \n",hashListEnt);

   /* check if entry is already deleted if yes then return OK */
   if((hashListEnt->list.next == NULL) ||
      (hashListEnt->list.prev == NULL))
      return(ROK);
   ...
   if(hashListEnt->key)
   {
   //LOG_PRINT(INFO,"----cmListDelete : free hashListEnt->key[%p] \n",hashListEnt->key);
     free(hashListEnt->key);
     hashListEnt->key       = NULL;
   }
...
)
{
...
#endif
   {
      ...
      ret = RFAILED;
      ret = cmHashMatchKey(hashListEnt->key, hashListEnt->keyLen, key, keyLen);
#ifdef DEBUG_ALL
	LOG_PRINT(INFO,"DSM_UTIL:cmHashListFind : cmHashMatchKey ret[%d]------\n",ret);
#endif
      if (ret == ROK)
      {
         /* matching key */
#ifdef DEBUG_ALL
	LOG_PRINT(INFO,"DSM_UTIL:cmHashListFind : Key MATCHED------\n");
#endif
         *entry = (U16 *) (((U8 *) hashListEnt) - hashListCp->offset);
         /* check for duplicates */
...
*              previous entry. If previous entry is NULL, gets first
...
)
{
   ...
   for (;;)
   {
      ...
      {
         /* found next entry */
         *entry = (U16 *) (((U8 *) prevListEnt) - hashListCp->offset);
