...
typedef	struct	qmodule {
	...
	int	(*messageservice)(void *mdata, void *pdata);
	void	*data;
...
*
* DESCRIPTION: 
*			Rearranging array of qpool pointers qd->alloc in qcontext 
*			in ascending order based on bufsize of each pool using 
*			standard selection sort algorithm
*
* ARGS:		
*			QMPOOL Q		Pointer to start of qpool pointer array in qcontext 
*						typecasted to void**
*			int n			Number of such valid pool pointers in array qd->alloc
*
* RETURNS:		Void	
*
******************************************************************************/
void 	qvPoolSort( QVMPOOLSET Q, int n )
{
	int lcv, rh, where;
	qpool *temp=NULL;   
	qpool **p=(qpool **)Q;
	...
	for( rh=n-1; rh>0; rh-- ) {
		...
		for ( lcv=1; lcv<=rh; lcv++ )
			if ( ((qpool *)(*(p + lcv)))->size > ((qpool *)(*(p + where)))->size )
				where = lcv;
		temp = *(p + where);
		*(p + where) = *(p + rh);
		*(p + rh) = temp;
...
#if 0
unsigned int
	__qvPoolStatus(QMPOOL pool)
{
	return ((qpool *)pool)->alloc;
}

unsigned int
	__qvHoldStatus(QDRIVER driver)
{
	return ysCount( &(((qvars *)driver)->hold) );
}

unsigned int
	__qvQueueStatus(QDRIVER driver)
{
	return ysCount( &(((qvars *)driver)->queue) );
}

static void	__rule(void)
{
	qvPrintf("=========================================================================\n");
}

static void	__tablerule(void)
{
	qvPrintf("--------------------------+----------------------------------------------\n");
}
...
void	__qvLibraryStatus( void )
{
	...
	__rule();
	qvLock( qd->lock );
	qvPrintf( "%25s | %s\n", "Driver Loop Wakeup", "Modules" );
	__tablerule();
	for( d = (qvars *)ylFirst( &(qd->driverloops) ); d; d = (qvars *)ylNext((YLNODE *)d) ) {
		qvLock( d->lock );
		W = __qvNextTime(d, &__wakeup , &(d->nextexpiry), 0);
		qvUnlock( d->lock );

		if( W ) {
			qvPrintf("%18lu.%06lu | ", W->s, W->us );
		} else {
			qvPrintf("%25s | ", "-");
		}

		for( N = ylFirst( &(d->modules) ); N; N = ylNext(N) ) {
			qmodule	*m = YMEMBEROF( qmodule, anchor.init, N );

			qvPrintf("%s ", m->identifier?m->identifier:"[Unknown]" );
		}
		qvPrintf( "\n" );
	}
	__tablerule();
	qvPrintf( "%25s | %s\n", "Service", "Served By" );
	__tablerule();
	for( T = ytFirst(&(qd->services), T_PREORDER ); T; T = ytNext(T, T_PREORDER) ) {
		qmodule	*m = YMEMBEROF( qmodule, anchor.service, T );
		qvPrintf("%25lu | ", m->name );
		if( m->scheduler ) {
			qvPrintf("M ");
			if( m->identifier ) {
				qvPrintf("%s", m->identifier);
			} else {
				qvPrintf("[Unknown] %08lX", (unsigned long)m );
			}
		} else if (m->dispatch) {
			qvPrintf("F %p", (void *)(m->dispatch) );
		} else {
			qvPrintf("Empty");
		}  
		qvPrintf( "\n" );
	...
	__tablerule();
	qvPrintf( "%25s | %s\n", "Pool Buffers", "Free/Total" );
	__tablerule();
	qvPrintf( "%25s | %u/%u\n", "Timer",
		ypCount( &((qpool *)(qd->timerpool))->list ),
		((qpool *)(qd->timerpool))->nbuf );
	qvPrintf( "%25s | %u/%u\n", "Module",
		ypCount( &((qpool *)(qd->modulepool))->list ),
		((qpool *)(qd->modulepool))->nbuf );
	qvPrintf( "%25s | %u/%u\n", "Packet Control",
		ypCount( &((qpool *)(qd->pctlpool))->list ),
		((qpool *)(qd->pctlpool))->nbuf );

	for( i=0; i<NVARPOOL; i++, size *= 2 ) {
		qvPrintf( "%25u | %u/%u\n", 
			size,
			ypCount( &((qpool *)(qd->alloc[i]))->list ), 
			((qpool *)(qd->alloc[i]))->nbuf );
	}
	__rule();
	qvPrintf("\n");
	qvUnlock( qd->lock );
